...
static enum DamageCategory GetReflectDamageMoveDamageCategory(enum BattlerId battler, u32 move)
{
...
static bool32 ShouldTeraShellDistortTypeMatchups(u32 move, enum BattlerId battlerDef, enum Ability abilityDef)
{
...
static inline bool32 IsDragonDartsSecondHit(enum BattlerId battlerAtk, u32 move)
{
...
bool32 IsHealBlockPreventingMove(enum BattlerId battler, u32 move)
{
...
bool32 IsBelchPreventingMove(enum BattlerId battler, u32 move)
{
...
//my change made timer bool 
//think just need put decrement here 
//vsonic
static enum MoveCanceler CancelerRecharge(struct BattleContext *ctx)
{
    ...
    {
        //attempt stop extra decrement from effects that 
        //would make battler act again
        if (!gSpecialStatuses[ctx->battlerAtk].dancerUsedMove
        && !gSpecialStatuses[ctx->battlerAtk].returnedBallMove)
            gBattleMons[ctx->battlerAtk].volatiles.rechargeTimer--;
...
//changed to work off hp threshold
...
//I moved healblock to side status
//prob would just requir renaming this
...
static enum MoveCanceler CancelerPPDeduction(struct BattleContext *ctx)
{
    ...
    if (gBattleStruct->submoveAnnouncement != SUBMOVE_NO_EFFECT)
    {
        ...
        else
        {
            ...
            //vsonic important - pr to address already recieved go over attempt port
            //hm I have ee I can just pull it in actually
...
static enum MoveCanceler CancelerWeatherPrimal(struct BattleContext *ctx)
{
    ...
    if (HasWeatherEffect() && GetMovePower(ctx->move) > 0)
    {
        ...
        if (moveType == TYPE_FIRE && gBattleWeather & WEATHER_RAIN_PRIMAL && (GetConfig(CONFIG_POWDER_STATUS_HEAVY_RAIN) >= GEN_7 || !TryActivatePowderStatus(ctx->move)))
        {
...
static enum MoveCanceler CancelerMoveFailure(struct BattleContext *ctx)
{
    ...
    case EFFECT_FUTURE_SIGHT:
        if (gBattleStruct->futureSight[ctx->battlerDef].counter2 > 0)
            battleScript = BattleScript_ButItFailed;
        break;
    ...
    case EFFECT_POLTERGEIST:
        ...
        break;
    case EFFECT_PROTECT:
        // TODO vsonic important
        //see if I can do anything for this, 
        //idea load odds store to battlestruct value
        //before move use idea use TryDoMoveEffectsBeforeMoves
        //could return pointer set to assign battlescript
        //if not fail return NULL
        //assign value and run odds in TryDoMoveEffectsBeforeMoves
        //return that value here in another function
        //TryReturnProtectFailScript
...
static enum MoveCanceler CancelerExplodingDamp(struct BattleContext *ctx)
{
    ...
    if ((dampBattler || IsFogOnField())
    && IsMoveDampBanned(ctx->move))
    {
...
//vsonic reworking explosion like effects
//only explosion itself will go off if faint
//others would fail if hp hits 0
//see if this is just targetting
//or activation if so may need to add on here
...
static enum MoveCanceler (*const sMoveSuccessOrderCancelers[])(struct BattleContext *ctx) =
{
    ...
    //put yawn here //look into same issue as recharge called moves instruct dancer vsonic
    ...
    [CANCELER_FOCUS_GEN5] = CancelerFocus,
    ...
    case ABILITYEFFECT_ENDTURN:
        if (IsBattlerAlive(battler))
        {
            ...
            case ABILITY_POWER_CONSTRUCT:
                ...
                break;
            case ABILITY_BALL_FETCH://vsonic
...
bool32 IsMoveMakingContact(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability abilityAtk, enum HoldEffect holdEffectAtk, u32 move)
{
    ...
    case ABILITY_PROTOSYNTHESIS:
        {
            ...
            if ((ctx->weather & WEATHER_SUN || gBattleMons[battlerDef].volatiles.boosterEnergyActivated)
...
static inline uq4_12_t ApplyOffensiveBadgeBoost(uq4_12_t modifier, enum BattlerId battler, u32 move)
{
...
static inline uq4_12_t ApplyDefensiveBadgeBoost(uq4_12_t modifier, enum BattlerId battler, u32 move)
{
...
static inline uq4_12_t GetMinimizeModifier(u32 move, enum BattlerId battlerDef)
{
...
static inline uq4_12_t GetUndergroundModifier(u32 move, enum BattlerId battlerDef)
{
...
static inline uq4_12_t GetDiveModifier(u32 move, enum BattlerId battlerDef)
{
...
static inline uq4_12_t GetAirborneModifier(u32 move, enum BattlerId battlerDef)
{
...
bool32 IsFutureSightAttackerInParty(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move)
{
...
static inline void TryNoticeIllusionInTypeEffectiveness(u32 move, enum Type moveType, enum BattlerId battlerAtk, enum BattlerId battlerDef, uq4_12_t resultingModifier, u32 illusionSpecies)
{
...
u16 GetBattleFormChangeTargetSpecies(enum BattlerId battler, enum FormChanges method)
{
    u32 i;
...
void SetDynamicMoveCategory(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move)
{
...
static u32 CanBattlerHitBothFoesInTerrain(enum BattlerId battler, u32 move, enum BattleMoveEffects effect)
{
...
u32 GetBattlerMoveTargetType(enum BattlerId battler, u32 move)
{
    enum BattleMoveEffects effect = GetMoveEffect(move);
...
bool32 CanMoveSkipAccuracyCalc(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability abilityAtk, enum Ability abilityDef, u32 move, enum FunctionCallOption option)
{
    bool32 effect = FALSE;
...
u32 GetTotalAccuracy(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move, enum Ability atkAbility, enum Ability defAbility, enum HoldEffect atkHoldEffect, enum HoldEffect defHoldEffect)
{
    u32 calc, moveAcc;
