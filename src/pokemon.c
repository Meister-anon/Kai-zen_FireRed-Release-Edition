...
s32 CalculateBaseDamage(struct BattlePokemon *attacker, struct BattlePokemon *defender, u32 move, u32 sideStatus, u16 powerOverride, u8 typeOverride, u8 battlerIdAtk, u8 battlerIdDef)
{
    ...
    if (WeatherHasEffect()) //weather dmg changes weren't working at all, think was becuz I had  below dmg calc
    {
...
bool8 PokemonUseItemEffects(struct Pokemon *mon, u16 item, u8 partyIndex, u8 moveIndex, bool8 usedByAI) //this "e" isn't wrong
{
    ...
    for (cmdIndex = 0; cmdIndex < 6; cmdIndex++)
    {
        ...
        case 3:
        {
            ...
            {
                data = gExperienceTables[gBaseStats[GetMonData(mon, MON_DATA_SPECIES)].growthRate][GetMonData(mon, MON_DATA_LEVEL) + 1];
    ...
    case EVO_MODE_NORMAL:
            ...
            //half write, needed both conditions,  as evolutions was specifically checking the method, not the i value
        {
            ...
            case EVO_MOVE_TYPE:
                for (j = 0; j < 4; j++)
                {
                    if (gBattleMoves[GetMonData(mon, MON_DATA_MOVE1 + j)].type == evolutions[i].param)
                    {
                        EVO_PRIORITY_CHECK(basePriority, GetEvoMethodPriority(evolutions[i].method));
                        targetSpecies = evolutions[i].targetSpecies;
                        break;
...
u8 GetPlayerPartyHighestLevel(void)
{
    ...
    {
        if (GetMonData(&gPlayerParty[slot], MON_DATA_SANITY_HAS_SPECIES) == 1 && !GetMonData(&gPlayerParty[slot], MON_DATA_SANITY_IS_EGG))
        {
...
static u16 GetDeoxysStat(struct Pokemon *mon, s32 statId)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_20 || GetMonData(mon, MON_DATA_SPECIES) != SPECIES_DEOXYS)
    {
        return statValue = 0;
    }
    else
...
u16 GetFormChangeTargetSpeciesBoxMon(struct Pokemon *mon, u16 method, u32 arg)
{
    ...
    if (formChanges != NULL)
    {
        ...
        ability = GetAbilityBySpecies(species, GetBoxMonData(boxMon, MON_DATA_ABILITY_NUM), mon);//uses boxmon instad of pokemon, but will still reach same data as I'm adding boxmon fields
