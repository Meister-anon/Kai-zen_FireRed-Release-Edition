...
static void PokedudeHandleGetMonData(enum BattlerId battler);
static void PokedudeHandleGetRawMonData(enum BattlerId battler);
static void PokedudeHandleSetMonData(enum BattlerId battler);
static void PokedudeHandleSetRawMonData(enum BattlerId battler);
static void PokedudeHandleLoadMonSprite(enum BattlerId battler);
static void PokedudeHandleSwitchInAnim(enum BattlerId battler);
static void PokedudeHandleReturnMonToBall(enum BattlerId battler);
...
static void PokedudeHandleTrainerSlideBack(enum BattlerId battler);
static void PokedudeHandleFaintAnimation(enum BattlerId battler);
static void PokedudeHandlePaletteFade(enum BattlerId battler);
static void PokedudeHandleSuccessBallThrowAnim(enum BattlerId battler);
static void PokedudeHandleBallThrowAnim(enum BattlerId battler);
static void PokedudeHandlePause(enum BattlerId battler);
static void PokedudeHandleMoveAnimation(enum BattlerId battler);
static void PokedudeHandlePrintString(enum BattlerId battler);
...
static void PokedudeHandleUnknownYesNoBox(enum BattlerId battler);
...
static void PokedudeHandleCmd23(enum BattlerId battler);
static void PokedudeHandleHealthBarUpdate(enum BattlerId battler);
static void PokedudeHandleExpUpdate(enum BattlerId battler);
static void PokedudeHandleStatusIconUpdate(enum BattlerId battler);
static void PokedudeHandleStatusAnimation(enum BattlerId battler);
static void PokedudeHandleStatusXor(enum BattlerId battler);
static void PokedudeHandleDataTransfer(enum BattlerId battler);
static void PokedudeHandleDMA3Transfer(enum BattlerId battler);
static void PokedudeHandlePlayBGM(enum BattlerId battler);
static void PokedudeHandleCmd32(enum BattlerId battler);
static void PokedudeHandleTwoReturnValues(enum BattlerId battler);
static void PokedudeHandleChosenMonReturnValue(enum BattlerId battler);
static void PokedudeHandleOneReturnValue(enum BattlerId battler);
static void PokedudeHandleOneReturnValue_Duplicate(enum BattlerId battler);
static void PokedudeHandleCmd37(enum BattlerId battler);
static void PokedudeHandleCmd38(enum BattlerId battler);
static void PokedudeHandleCmd39(enum BattlerId battler);
static void PokedudeHandleCmd40(enum BattlerId battler);
static void PokedudeHandleHitAnimation(enum BattlerId battler);
static void PokedudeHandleCmd42(enum BattlerId battler);
static void PokedudeHandlePlaySE(enum BattlerId battler);
static void PokedudeHandlePlayFanfare(enum BattlerId battler);
static void PokedudeHandleFaintingCry(enum BattlerId battler);
static void PokedudeHandleIntroSlide(enum BattlerId battler);
...
static void PokedudeHandleHidePartyStatusSummary(enum BattlerId battler);
static void PokedudeHandleEndBounceEffect(enum BattlerId battler);
static void PokedudeHandleSpriteInvisibility(enum BattlerId battler);
static void PokedudeHandleBattleAnimation(enum BattlerId battler);
static void PokedudeHandleLinkStandbyMsg(enum BattlerId battler);
static void PokedudeHandleResetActionMoveSelection(enum BattlerId battler);
static void PokedudeHandleCmd55(enum BattlerId battler);
static void PokedudeCmdEnd(enum BattlerId battler);
...
static void PokedudeBufferExecCompleted(enum BattlerId battler);
...
static u32 CopyPokedudeMonData(enum BattlerId battler, u8 monId, u8 *dst);
static void SetPokedudeMonData(enum BattlerId battler, u8 monId);
static void StartSendOutAnim(enum BattlerId battlerId);
static void PokedudeDoMoveAnimation(enum BattlerId battler);
...
// not used
static const u8 gUnknown_8479000[] = { 0x48, 0x48, 0x0, 0x5a, 0x50, 0x50, 0x50, 0x58 };
...
static void PokedudeDummy(enum BattlerId battler)
{
}
...
static u32 CopyPokedudeMonData(enum BattlerId battler, u8 monId, u8 *dst)
{
    ...
    case REQUEST_MOVE4_BATTLE:
        data16 = GetMonData(mon, MON_DATA_MOVE1 + gBattleResources->bufferA[battler][1] - REQUEST_MOVE1_BATTLE);
        dst[0] = data16;
        dst[1] = data16 >> 8;
        size = 2;
        break;
    ...
    case REQUEST_PPMOVE4_BATTLE:
        dst[0] = GetMonData(mon, MON_DATA_PP1 + gBattleResources->bufferA[battler][1] - REQUEST_PPMOVE1_BATTLE);
        size = 1;
        break;
...
static void SetPokedudeMonData(enum BattlerId battler, u8 monId)
{
    ...
    case REQUEST_ALL_BATTLE:
    {
        ...
        break;
    case REQUEST_SPECIES_BATTLE:
        SetMonData(mon, MON_DATA_SPECIES, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_HELDITEM_BATTLE:
        SetMonData(mon, MON_DATA_HELD_ITEM, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_MOVES_PP_BATTLE:
        for (i = 0; i < MAX_MON_MOVES; ++i)
        {
    ...
    case REQUEST_MOVE3_BATTLE:
    case REQUEST_MOVE4_BATTLE:
        SetMonData(mon, MON_DATA_MOVE1 + gBattleResources->bufferA[battler][1] - REQUEST_MOVE1_BATTLE, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_PP_DATA_BATTLE:
        SetMonData(mon, MON_DATA_PP1, &gBattleResources->bufferA[battler][3]);
        SetMonData(mon, MON_DATA_PP2, &gBattleResources->bufferA[battler][4]);
        SetMonData(mon, MON_DATA_PP3, &gBattleResources->bufferA[battler][5]);
        SetMonData(mon, MON_DATA_PP4, &gBattleResources->bufferA[battler][6]);
        SetMonData(mon, MON_DATA_PP_BONUSES, &gBattleResources->bufferA[battler][7]);
        break;
    case REQUEST_PPMOVE1_BATTLE:
    case REQUEST_PPMOVE2_BATTLE:
    case REQUEST_PPMOVE3_BATTLE:
    case REQUEST_PPMOVE4_BATTLE:
        SetMonData(mon, MON_DATA_PP1 + gBattleResources->bufferA[battler][1] - REQUEST_PPMOVE1_BATTLE, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_OTID_BATTLE:
        SetMonData(mon, MON_DATA_OT_ID, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_EXP_BATTLE:
        SetMonData(mon, MON_DATA_EXP, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_HP_EV_BATTLE:
        SetMonData(mon, MON_DATA_HP_EV, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_ATK_EV_BATTLE:
        SetMonData(mon, MON_DATA_ATK_EV, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_DEF_EV_BATTLE:
        SetMonData(mon, MON_DATA_DEF_EV, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_SPEED_EV_BATTLE:
        SetMonData(mon, MON_DATA_SPEED_EV, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_SPATK_EV_BATTLE:
        SetMonData(mon, MON_DATA_SPATK_EV, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_SPDEF_EV_BATTLE:
        SetMonData(mon, MON_DATA_SPDEF_EV, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_FRIENDSHIP_BATTLE:
        SetMonData(mon, MON_DATA_FRIENDSHIP, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_MET_LOCATION_BATTLE:
        SetMonData(mon, MON_DATA_MET_LOCATION, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_MET_LEVEL_BATTLE:
        SetMonData(mon, MON_DATA_MET_LEVEL, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_MET_GAME_BATTLE:
        SetMonData(mon, MON_DATA_MET_GAME, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_POKEBALL_BATTLE:
        SetMonData(mon, MON_DATA_POKEBALL, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_ALL_IVS_BATTLE:
        SetMonData(mon, MON_DATA_HP_IV, &gBattleResources->bufferA[battler][3]);
        SetMonData(mon, MON_DATA_ATK_IV, &gBattleResources->bufferA[battler][4]);
        SetMonData(mon, MON_DATA_DEF_IV, &gBattleResources->bufferA[battler][5]);
        SetMonData(mon, MON_DATA_SPEED_IV, &gBattleResources->bufferA[battler][6]);
        SetMonData(mon, MON_DATA_SPATK_IV, &gBattleResources->bufferA[battler][7]);
        SetMonData(mon, MON_DATA_SPDEF_IV, &gBattleResources->bufferA[battler][8]);
        break;
    case REQUEST_HP_IV_BATTLE:
        SetMonData(mon, MON_DATA_HP_IV, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_ATK_IV_BATTLE:
        SetMonData(mon, MON_DATA_ATK_IV, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_DEF_IV_BATTLE:
        SetMonData(mon, MON_DATA_DEF_IV, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_SPEED_IV_BATTLE:
        SetMonData(mon, MON_DATA_SPEED_IV, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_SPATK_IV_BATTLE:
        SetMonData(mon, MON_DATA_SPATK_IV, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_SPDEF_IV_BATTLE:
        SetMonData(mon, MON_DATA_SPDEF_IV, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_PERSONALITY_BATTLE:
        SetMonData(mon, MON_DATA_PERSONALITY, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_CHECKSUM_BATTLE:
        SetMonData(mon, MON_DATA_CHECKSUM, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_STATUS_BATTLE:
        SetMonData(mon, MON_DATA_STATUS, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_LEVEL_BATTLE:
        SetMonData(mon, MON_DATA_LEVEL, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_HP_BATTLE:
        SetMonData(mon, MON_DATA_HP, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_MAX_HP_BATTLE:
        SetMonData(mon, MON_DATA_MAX_HP, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_ATK_BATTLE:
        SetMonData(mon, MON_DATA_ATK, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_DEF_BATTLE:
        SetMonData(mon, MON_DATA_DEF, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_SPEED_BATTLE:
        SetMonData(mon, MON_DATA_SPEED, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_SPATK_BATTLE:
        SetMonData(mon, MON_DATA_SPATK, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_SPDEF_BATTLE:
        SetMonData(mon, MON_DATA_SPDEF, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_COOL_BATTLE:
        SetMonData(mon, MON_DATA_COOL, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_BEAUTY_BATTLE:
        SetMonData(mon, MON_DATA_BEAUTY, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_CUTE_BATTLE:
        SetMonData(mon, MON_DATA_CUTE, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_SMART_BATTLE:
        SetMonData(mon, MON_DATA_SMART, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_TOUGH_BATTLE:
        SetMonData(mon, MON_DATA_TOUGH, &gBattleResources->bufferA[battler][3]);
        break;
    /*case REQUEST_SHEEN_BATTLE:
        SetMonData(mon, MON_DATA_SHEEN, &gBattleResources->bufferA[battler][3]);
        break;*/
    case REQUEST_COOL_RIBBON_BATTLE:
        SetMonData(mon, MON_DATA_COOL_RIBBON, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_BEAUTY_RIBBON_BATTLE:
        SetMonData(mon, MON_DATA_BEAUTY_RIBBON, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_CUTE_RIBBON_BATTLE:
        SetMonData(mon, MON_DATA_CUTE_RIBBON, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_SMART_RIBBON_BATTLE:
        SetMonData(mon, MON_DATA_SMART_RIBBON, &gBattleResources->bufferA[battler][3]);
        break;
    case REQUEST_TOUGH_RIBBON_BATTLE:
        SetMonData(mon, MON_DATA_TOUGH_RIBBON, &gBattleResources->bufferA[battler][3]);
        break;
    }
    HandleLowHpMusicChange(&gPlayerParty[gBattlerPartyIndexes[battler]], battler);
}
...
static void PokedudeHandleDrawTrainerPic(enum BattlerId battler)
{
    if (GetBattlerSide(battler) == B_SIDE_PLAYER)
    {
        DecompressTrainerBackPalette(BACK_PIC_POKEDUDE, battler);
        SetMultiuseSpriteTemplateToTrainerBack(BACK_PIC_POKEDUDE, GetBattlerPosition(battler));
        gBattlerSpriteIds[battler] = CreateSprite(&gMultiuseSpriteTemplate,
                                                         80,
...
//previously used *(&gBattleStruct->battlerPreventingSwitchout) = (IsAbilityPreventingEscape(battler) - 1);
//unsure what correct vsonic important
static void PokedudeHandleChoosePokemon(enum BattlerId battler)
{
    s32 i;
