...
static EWRAM_DATA u8 (*sListMenuItemStrings)[25] = NULL; //was 19 change for longer item names & font adjust setup
...
static bool8 LoadBagMenuGraphics(void)
{
    ...
    case 14:
        ...
        gTasks[taskId].data[0] = ListMenuInit(&gMultiuseListMenuTemplate, gBagMenuState.cursorPos[gBagMenuState.pocket], gBagMenuState.itemsAbove[gBagMenuState.pocket], DEFAULT_MODE);
...
static void PrintItemDescriptionOnMessageWindow(s32 itemIndex)
{
    ...
    BagPrintTextOnWindow(1, 2, description, 0, 3, 0, 0, 0, 0);
}
...
static void Task_BagMenu_HandleInput(u8 taskId)
{
    ...
    input = ListMenu_ProcessInput(data[0], DEFAULT_MODE); //cap in bag already eworks so believe can safely use default?
...
static void Task_AnimateSwitchPockets(u8 taskId)
{
    ...
    case 1:
        ...
        PrintBagPocketName();
        Bag_BuildListMenuTemplate(gBagMenuState.pocket);
        data[0] = ListMenuInit(&gMultiuseListMenuTemplate, gBagMenuState.cursorPos[gBagMenuState.pocket], gBagMenuState.itemsAbove[gBagMenuState.pocket], DEFAULT_MODE);
        PutWindowTilemap(1);
        PutWindowTilemap(2);
        ScheduleBgCopyTilemapToVram(0);
        CreatePocketScrollArrowPair();
...
static void UpdateQuantityToTossOrDeposit(s16 value, u8 ndigits)
{
    ...
    StringExpandPlaceholders(gStringVar4, gText_TimesStrVar1);
    BagPrintTextOnWindow(r6, 0, gStringVar4, 4, 10, 0, 0, 0, 1);
}
...
static void OpenContextMenu(u8 taskId)
{
    ...
    BagPrintTextOnWindow(r4, 2, gStringVar4, 0, 2, 0, 0, 0, 1);
}
...
static void Task_ConfirmTossItems(u8 taskId)
{
    ...
    BagPrintTextOnWindow(ShowBagWindow(6, 1), 2, gStringVar4, 0, 2, 0, 0, 0, 1);
...
static void Task_InitSaleQuantitySelectInterface(u8 taskId)
{
    ...
    StringExpandPlaceholders(gStringVar4, gText_TimesStrVar1);
    BagPrintTextOnWindow(r4, 0, gStringVar4, 4, 10, 0, 0, 0xFF, 1);
