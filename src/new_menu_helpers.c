...
static const struct FontInfo gFontInfos[] = 
{
    ...
    //lowered appostrophe so doesn't cut off stuff will need to do for new fonts I add in
    [FONT_NARROW] = {
...
void AddTextPrinterDiffStyle(bool8 allowSkippingDelayWithButtonPress)
{
    ...
    if (result == 0)
        AddTextPrinterParameterized2(0, 4, gStringVar4, GetPlayerTextSpeedDelay(), nptr, 8, 1, 3);
    else if (result == 1)
        AddTextPrinterParameterized2(0, 5, gStringVar4, GetPlayerTextSpeedDelay(), nptr, 4, 1, 3);
    else
        AddTextPrinterParameterized2(0, 2, gStringVar4, GetPlayerTextSpeedDelay(), nptr, 2, 1, 3);
}
...
//fontId  its passing font text speed is set below by thefunction
//vsonic IMPORTANT
//wasfixed in pret, so track commit to find fix
void DisplayItemMessageOnField(u8 taskId, u8 textSpeed, const u8 *string, TaskFunc callback)
{
    LoadStdWindowFrameGfx();
    DisplayMessageAndContinueTask(taskId, 0, DLG_WINDOW_BASE_TILE_NUM, DLG_WINDOW_PALETTE_NUM, textSpeed, GetPlayerTextSpeedDelay(), string, callback);
