...
#define POKESUM_WIN_LVL_NICK         2  //lvl and name displayed to win_info box
...
#define POKESUM_WIN_INFO_4           4  //trainer memo
...
{
    u8 tempStr[POKEMON_NAME_BUFFER_SIZE]; //expanded from 20, to account for longer names plus buffer
    u16 dexNum;
    u16 gender;
    u16 heldItem;
    u32 otId;
...
static void BufferMonMoveI(u8 i)//think this is the menu/function I need has move index and pp //transform need change
{
    ...
    //why am I effing stupid? just remove the gmovenmaes blocks from this
    if (sMonSummaryScreen->moveIds[i] == MOVE_HIDDEN_POWER)
        power = hiddenpower;
    else
        power = gMovesInfo[sMonSummaryScreen->moveIds[i]].power;
...
//is main window in summ screen where front sprite is shown
...
static void PrintMonLevelNickOnWindow2(const u8 * str)
{
    ...
    if (!sMonSummaryScreen->isEgg)  //is not an egg
    {
        if (sMonSummaryScreen->curPageIndex != PSS_PAGE_MOVES_INFO) //should remove gender symbol from move info page
        {
            ...
            AddTextPrinterParameterized3(sMonSummaryScreen->windowIds[POKESUM_WIN_LVL_NICK], FONT_NORMAL, 35, 2, sLevelNickTextColors[1], TEXT_SKIP_DRAW, sMonSummaryScreen->summary.nicknameStrBuf);
...
{                   //pretty sure this is trainer memo screen used for your caught mon
    ...
    if (level == 0)
        level = 5; //set just in case,but should still never encounter this as changed hatch logic
    ...
    {
        if (GetMonData(&sMonSummaryScreen->currentMon, MON_DATA_EVENT_LEGAL) == 1) // Fateful encounter //removed field so never true
        {
...
static void PokeSum_PrintTrainerMemo_Mon_NotHeldByOT(void)  //believe this is traded/ not originally yours
{
    ...
    if (level == 0)
        ...
        //changed so shouldn't need  -kept just in case
...
static void PokeSum_PrintAbilityNameAndDesc(void)
{
    ...
    AddTextPrinterParameterized3(sMonSummaryScreen->windowIds[5], GetFontIdToFit(sMonSummaryScreen->summary.abilityNameStrBuf, FONT_NORMAL, 0, 72), 
        64, 0, sLevelNickTextColors[0], TEXT_SKIP_DRAW, sMonSummaryScreen->summary.abilityNameStrBuf);
...
static u16 GetMonPpByMoveSlot(struct Pokemon * mon, u8 moveSlot)
{
    ...
    u32 curr_personality, slot1_personality, slot2_personality, slot3_personality;
    u32 singles, doubles, rotation;
    curr_personality = GetMonData(&sMonSummaryScreen->currentMon, MON_DATA_PERSONALITY); //is same thing but could replace curentmon with mon
    slot1_personality = gBattleMons[GetBattlerAtPosition(B_POSITION_PLAYER_LEFT)].personality;
    slot2_personality = gBattleMons[GetBattlerAtPosition(B_POSITION_PLAYER_RIGHT)].personality;
    slot3_personality = GetMonData(&gPlayerParty[2], MON_DATA_PERSONALITY); //placehold value setup later
...
static void CreateBallIconObj(void)
{
    ...
    if (!sMonSummaryScreen->isEgg)
        ...
        ballItemId = 0; //base pokeball so use this for dex
...
static void UpdateMonStatusIconObj(void)//important
{
    ...
    //change field to aray, hold max set status loop over each slot and have field counting num set status
    //if stauts none, or not set go to invisible here
    //use num applied ailments to determine the slot position
    //for health box bounce between different by fading to ailment none
    //before swapping to next applied ailment using this field
    //think just want to set 2 status, with certain statuses curing the other
    if (sMonSummaryScreen->curMonStatusAilment == AILMENT_NONE) 
    {
...
{
    if (sMonSummaryScreen->curMonStatusAilment == AILMENT_NONE || sMonSummaryScreen->isEgg) // || sMonSummaryScreen->curPageIndex == PSS_PAGE_MOVES_INFO
    ...
    if (sMonSummaryScreen->curPageIndex == PSS_PAGE_MOVES_INFO)
    {
        ...
        {
            sStatusIcon->sprite->invisible = TRUE; //nvm just need this
            sStatusIcon->sprite->x = 16; //important this unknown and data may be for position of status icon when on move page.
            sStatusIcon->sprite->y = 45;
            return;
        }//think remove this cux of my info page changes
    }
    else if (sStatusIcon->sprite->y != 38) //and this may be for every other windows.
    {
