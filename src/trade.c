...
static void CB2_ReturnFromLinkTrade2(void)
{
    ...
    case 7:
        ...
        for (i = 0; i < sTradeMenuResourcesPtr->partyCounts[0]; i++)
        {
            ...
            sTradeMenuResourcesPtr->partyIcons[0][i] = CreateMonIcon(GetMonData(mon, MON_DATA_SPECIES_OR_EGG),
                                                                SpriteCB_MonIcon,
...
//is DrawSelectedMonScreen in EE
static void HandleRedrawTradeMenuOnSide(u8 side)
{
    s8 nameStringWidth;
    ...
    case 1:
        ...
        Trade_MoveSelectedMonToTarget(&gSprites[sTradeMenuResourcesPtr->partyIcons[0][partyIdx + (whichParty * PARTY_SIZE)]]);
...
static u32 TestWhetherSelectedMonCanBeTraded(struct Pokemon * party, int partyCount, int monIdx)
{
    ...
    for (i = 0; i < partyCount; i++)
    {
        species2[i] = GetMonData(&party[i], MON_DATA_SPECIES_OR_EGG);
