...
struct //must match order of constants in save.h
{
    u32 offset;  //changed to u32 from u16 to get more box space
    u16 size;
...
static u8 CopySaveSlotData(u16 sectorId, const struct SaveSectorLocation *locations)
{
    ...
    for (i = 0; i < NUM_SECTORS_PER_SLOT; i++)
    {
        ReadFlashSector(i, gSaveDataBufferPtr);
...
u8 HandleSavingData(u8 saveType)
{
    ...
    case SAVE_EREADER: // unused
        SaveSerializedGame();
        // only SaveBlock2
        WriteSaveSectorOrSlot(SECTOR_ID_SAVEBLOCK2_START, gRamSaveSectorLocations);
        break;
