...
u8 CreatePartyStatusSummarySprites(enum BattlerId battlerId, struct HpAndStatus *partyInfo, bool8 isSwitchingMons, bool8 isBattleStart)
{
    bool8 isOpponent;
    s8 nValidMons;
    s16 bar_X, bar_Y, bar_pos2_X, speed;
    s32 i, j, var;
...
void UpdateNickInHealthbox(u8 healthboxSpriteId, struct Pokemon *mon)
{
    ...
    if (StringCompare(gSpeciesInfo[species].speciesName, nickname) == IDENTICAL) //if not nicknamed reassign tempStr to speciesname, making it update capitalization
    ...
    *ptr++ = EXT_CTRL_CODE_COLOR; //couldn't figure out how to do font change here
    ...
    if ((species == SPECIES_NIDORAN_F || species == SPECIES_NIDORAN_M) && StringCompare(nickname, gSpeciesInfo[species].speciesName) == IDENTICAL)
        gender = 100;
    ...
    //realized this uses 55 as limit as w font small letter characters are all 5 px wide, meaning this caps at name length 11
    //which is the max that fits
