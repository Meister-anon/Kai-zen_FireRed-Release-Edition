...
EWRAM_DATA u16 gPartnerTrainerId = 0;           //added also for EE, believe for battle where player has ai trainer as partner
...
static u8 GetTrainerBattleTransition(void)
{
    ...
    if (gTrainers[gTrainerBattleOpponent_A].trainerClass == CLASS_ELITE_FOUR_FRLG)
    {
    ...
    if (gTrainers[gTrainerBattleOpponent_A].trainerClass == CLASS_CHAMPION_FRLG)
        return B_TRANSITION_BLUE;
