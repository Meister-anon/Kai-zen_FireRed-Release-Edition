...
//need update with my sleep heal, early bird logic, and my freeze effect
//defrost change to frostbite
...
static enum CancelerResult CancelerPPDeduction(struct BattleContext *ctx)
{
    ...
    if (gBattleStruct->submoveAnnouncement != SUBMOVE_NO_EFFECT)
    {
        ...
        else
        {
            ...
            SetTypeBeforeUsingMove(ctx->move, ctx->battlerAtk, &gBattleStruct->dynamicMoveType);
...
static enum CancelerResult CancelerWeatherPrimal(struct BattleContext *ctx)
{
    ...
    if (GetMovePower(ctx->move) > 0 && HasWeatherEffect())
    {
        ...
        if (moveType == TYPE_FIRE && gBattleWeather & WEATHER_RAIN_PRIMAL && (GetConfig(B_POWDER_STATUS_HEAVY_RAIN) >= GEN_7 || !TryActivatePowderStatus(ctx->move)))
        {
...
static enum CancelerResult CancelerMoveFailure(struct BattleContext *ctx)
{
    ...
    case EFFECT_AURORA_VEIL:
        if (!(gBattleWeather & WEATHER_COLD_ANY && HasWeatherEffect()))
            battleScript = BattleScript_ButItFailed;
        break;
...
//idk why 50% recoil is in absorb prob need 
//go back and readd my custom stuff
//make sure recoil is right smh
    ...
    do
    {
        ...
        case FAINT_BLOCK_TRY_DESTINY_BOND: // Checked before FAINT_BLOCK_FAINT_TARGET but occurs after since volatiles are cleared on faint
            if (gBattleMons[gBattlerTarget].volatiles.destinyBond
             ...
             && !IsBattlerAlly(gBattlerAttacker, gBattlerTarget))
            {
                gBattleStruct->tryDestinyBond = TRUE;//think should put my sturdy rework here?
            }
            gBattleStruct->eventState.moveEndBlock++;
            break;
        ...
        case FAINT_BLOCK_FAINT_TARGET:
            ...
            //keep track think don't want evo method but research see what is
...
static enum MoveEndResult MoveEnd_MoveBlock(void)
{
    ...
    case EFFECT_SMACK_DOWN:
        if (!IsBattlerGrounded_IgnoreException(gBattlerTarget, GetBattlerAbility(gBattlerTarget), GetBattlerHoldEffect(gBattlerTarget))
         && IsBattlerTurnDamaged(gBattlerTarget)
         && IsBattlerAlive(gBattlerTarget)
         && !DoesSubstituteBlockMove(gBattlerAttacker, gBattlerTarget, gCurrentMove))
        {
...
//most likely remove from here for new effect
...
static enum MoveEndResult MoveEnd_FetchBall(void)
{
    enum MoveEndResult result = MOVEEND_RESULT_CONTINUE;

    if (IsDanceMove(gCurrentMove) && !gBattleStruct->snatchedMoveIsUsed)
    {
        enum BattlerId battler, nextFetchUser = 0;
        bool32 hasBallFetchTriggered = FALSE;

        for (battler = 0; battler < gBattlersCount; battler++)
        {
            if (gSpecialStatuses[battler].returnedBallMove)
            {
                // in case a battler fails to act on a Fetch-called move
                hasBallFetchTriggered = TRUE;
                break;
            }
        }

        if (!(!IsAnyTargetAffected()
         || (gBattleStruct->unableToUseMove && !hasBallFetchTriggered)
         || (!gSpecialStatuses[gBattlerAttacker].returnedBallMove && gBattleStruct->bouncedMoveIsUsed)))
        {   // Dance move succeeds
            // Set target for other Dancer mons; set bit so that mon cannot activate Dancer off of its own move
            ...
            for (battler = 0; battler < gBattlersCount; battler++)
            {
                ...
                {
                    if (!nextFetchUser || (gBattleMons[battler].speed < gBattleMons[nextFetchUser & 0x3].speed))
                        nextFetchUser = battler | 0x4;
                }
            }
            if (nextFetchUser && AbilityBattleEffects(ABILITYEFFECT_MOVE_END_OTHER, nextFetchUser & 0x3, 0, gCurrentMove, TRUE))
                result = MOVEEND_RESULT_RUN_SCRIPT;
        }
    }
...
static enum MoveEndResult (*const sMoveEndHandlers[])(void) =
{
    ...
    [MOVEEND_FETCH_BALL] = MoveEnd_FetchBall,
...
// Utility functions
//assert still to fix/setup vsonic important
static void ValidateBattlers(void)
{
...
//CANCELER from battle_util
//has more of my custom changes go over and think replace
//identical ones in this file with this
/*
...
{
    enum MoveCanceler effect = CANCELER_RESULT_BREAK;
    ...
    if (gBattleMons[ctx->battlerAtk].status1 & STATUS1_SLEEP)
    {
        ...
        else
        {
            ...
            if (gBattleMons[ctx->battlerAtk].status1 & STATUS1_SLEEP)
            {
                ...
                {
                    effect = CANCELER_RESULT_FAILURE;
                    gBattlescriptCurrInstr = BattleScript_MoveUsedIsAsleep;
                }
                else 
                {
...
{
    enum MoveCanceler effect = CANCELER_RESULT_SUCCESS;
    if (HasWeatherEffect() && GetMovePower(ctx->move) > 0)
    {
        ...
        if (effect == CANCELER_RESULT_FAILURE)
        {
            gProtectStructs[ctx->battlerAtk].chargingTurn = FALSE;
            CancelMultiTurnMoves(ctx->battlerAtk, SKY_DROP_ATTACKCANCELER_CHECK);
            gBattlescriptCurrInstr = BattleScript_PrimalWeatherBlocksMove;
...
static enum MoveCanceler CancelerCharging(struct BattleContext *ctx)
{
    ...
    enum MoveCanceler step = CANCELER_RESULT_SUCCESS;
