...
void RecordMixTrainerNames(void)
{
    if (gWirelessCommType != 0)
    {
        ...
        for (i = 0; i < GetLinkPlayerCount(); i++)
        {
            ...
            for (j = 0; j < 20; j++)
            {
                if ((u16)gLinkPlayers[i].trainerId ==  gSaveBlock1Ptr->trainerNameRecords[j].trainerId && StringCompare(gLinkPlayers[i].name, gSaveBlock1Ptr->trainerNameRecords[j].trainerName) == IDENTICAL)
                {
...
bool32 PlayerHasMetTrainerBefore(u16 id, u8 *name)
{
    ...
    for (i = 0; i < 20; i++)
    {
        if (StringCompareN(gSaveBlock1Ptr->trainerNameRecords[i].trainerName, name, 7) == IDENTICAL && gSaveBlock1Ptr->trainerNameRecords[i].trainerId == id)
        {
            return TRUE;
        }
        if (NameIsEmpty(gSaveBlock1Ptr->trainerNameRecords[i].trainerName))
