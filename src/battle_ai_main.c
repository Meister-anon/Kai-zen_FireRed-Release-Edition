...
static s32 AI_CheckBadMove(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, s32 score)
{
    ...
    if (IsPowderMove(move) && !IsAffectedByPowderMove(battlerDef, abilityDef, abilityAtk, aiData->holdEffects[battlerDef]))
    ...
    if (((GetMoveType(move) == TYPE_FIRE && GetMovePower(move) != 0) || CanBurnHitThaw(move)) && effectiveness < SUPER_EFFECTIVE && (gBattleMons[battlerDef].status1 & STATUS1_FREEZE_OR_FROSTBITE))
    {
        u32 aiMove;
        for (u32 moveIndex = 0; moveIndex < MAX_MON_MOVES; moveIndex++)
        {
...
// the following checks apply to any target (including user)
    ...
    // other
        ...
        case EFFECT_CONVERSION:
            ...
            break;
        case EFFECT_REST:
            if (!CanBeSlept(battlerAtk, battlerAtk, ABILITY_NONE, aiData->abilities[battlerAtk], NOT_BLOCKED_BY_SLEEP_CLAUSE))
                ADJUST_SCORE(-10);
            //fallthrough
    ...
    if (moveTarget == TARGET_FOES_AND_ALLY)
    {
        ...
        // Partner will not faint.
            ...
            case HOLD_EFFECT_WEAKNESS_POLICY:
                if (aiData->effectiveness[battlerAtk][battlerAtkPartner][gAiThinkingStruct->movesetIndex] >= SUPER_EFFECTIVE && isFriendlyFireOK)
                {
                    ADJUST_SCORE(GOOD_EFFECT);
                }
...
static s32 AI_CalcMoveEffectScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, struct AiLogicData *aiData)
{
    ...
    case EFFECT_HAZE:
        ...
        break;
    case EFFECT_WHIRLWIND: //believe need add tailwind logic here for plus1 score? vsonic important
    case EFFECT_ROAR:
    ...
    case EFFECT_LASER_FOCUS:
        if (aiData->abilities[battlerAtk] == ABILITY_SUPER_LUCK
         || aiData->abilities[battlerAtk] == ABILITY_GOLDENEYE
    ...
    case EFFECT_FOLLOW_ME:
        if (hasPartner
          ...
          && !IsBattlerIncapacitated(battlerDef, aiData->abilities[battlerDef])
          && (!IsPowderMove(move) || IsAffectedByPowderMove(battlerDef, aiData->abilities[battlerDef], aiData->abilities[battlerAtk], aiData->holdEffects[battlerDef])))
