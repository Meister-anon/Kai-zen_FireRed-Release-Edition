...
//EWRAM_DATA static bool8 sAnimHideHpBoxes = FALSE; vsonic dont add this is ee only config feature
...
const struct CompressedSpritePalette gBattleAnimPaletteTable[] =
{
    ...
    {gBattleAnimSpritePal_TriForceTriangle, ANIM_TAG_TRI_ATTACK_TRIANGLE},
    ...
    {NULL, ANIM_TAG_UNAVAILABLE_1}, //don't know why here, but kept from emerald
    ...
    {gBattleAnimSpritePal_Poltergeist, ANIM_TAG_POLTERGEIST}, //last added value
...
//search gBattleAnimBgImage_(),
//gBattleAnimBgPalette_(),
//&
//gBattleAnimBgTilemap_(),
const struct BattleAnimBackground gBattleAnimBackgroundTable[] =
{
    ...
    [BG_HIGH_SPEED]                 = {gBattleAnimBgImage_Hurricane,               gBattleAnimBgPalette_Hurricane,               gBattleAnimBgTilemap_Hurricane},
...
static const u8* const sBattleAnims_StatusConditions[NUM_B_ANIMS_STATUS] =
{
    ...
    [B_ANIM_STATUS_INFESTED]    = gBattleAnimStatus_Infestation,
    [B_ANIM_POWDER]             = gBattleAnimStatus_Powder, //can't remember what for
...
static const u8* const sBattleAnims_General[NUM_B_ANIMS_GENERAL] =
{
    ...
    [B_ANIM_ITEM_EFFECT]            = gBattleAnimGeneral_HeldItemEffect,
    ...
    [B_ANIM_SPIKE_TOSS]             = gBattleAnimGeneral_Spikes,
    [B_ANIM_BASIC_HEAL]             = gBattleAnimGeneral_Heal_Animation,
    ...
    [B_ANIM_TARGET_SCARED]          = gBattleAnimGeneral_TargetMonScared,
    [B_ANIM_INFESTATION]            = gBattleAnimStatus_Infestation,
    [B_ANIM_MOONLIGHT_SHINES]       = gBattleAnimGeneral_Moonlight,
    ...
    //new starting status field effects? 
...
void LaunchBattleAnimation(u32 animType, u32 animId)
{
    ...
    if (animType == ANIM_TYPE_MOVE)
    {
        for (i = 0; sMovesWithQuietBGM[i] != 0xFFFF; i++)
        {
...
//it throws off the balance of the move overall,
//instead must be done the way pokieabbie did it
//long as I keep it to 2, it seems to be fine?
//or could just be needing to keep it perfectly linked with other
//delayreductions
#define BATTLE_ANIM_DELAY_CMD
static void ScriptCmd_delay(void) //only this one need use actual speed return as its a multiple the otehrs are additive
{
    u8 BattleSpeed = Rogue_GetBattleSpeedScale();
    sBattleAnimScriptPtr++;
    sAnimFramesToWait = sBattleAnimScriptPtr[0];

    if (GetBattleSpeedOption > OPTIONS_BATTLE_SPEED_1X)     //delay 1 in bs command is used as buffer to clear windows etc. so battle speedup in battle_main skips frames breaking that         
    {    
        //fix to ensure no loss of anim buffer clears
        //Pokabbies new setup has fix that stops speedup if loading palette fade
        //which may make this "fix" unnecessary but delay 1 is used in otehr places too
        //so just keep an eye out and see, but her version doesn't have logic here in delay
        
        //turned off for first test - yup  didn't need this with Pokabbies new fix/
        //if (sAnimFramesToWait == 1)
        //    sAnimFramesToWait *= SQUARE(BattleSpeed); //think should work, issue is num frames moved by battle speed up, 
            //so if delay is for buffer need link to that if loop that 3 times multiply delay by 3 so it remains unchanged
            //ok almost had it , forgot I'm cutting it in half below, so what I need
            //is to do DOUBLE what the number of times I'm looping or num frame loops, * num divided below
            //for me 2 loops * divisor of 2  so 4  and that worked
