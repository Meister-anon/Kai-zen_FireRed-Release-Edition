...
static const s8 sAiAbilityRatings[ABILITIES_COUNT] =
{
    ...
    [ABILITY_TRIAGE] = 7,
    [ABILITY_TRUANT] = -2,
    [ABILITY_TURBOBLAZE] = 8,
    [ABILITY_UNAWARE] = 6,
    [ABILITY_UNBURDEN] = 7,
    [ABILITY_UNNERVE] = 7,
    [ABILITY_VICTORY_STAR] = 7,
...
static const u16 sEncouragedEncoreEffects[] =
{
    ...
    EFFECT_ATTACK_UP_2,
    EFFECT_ENCORE,
    //EFFECT_CONVERSION_2, //reworked consolidated effects via command logic
    EFFECT_LOCK_ON,
    EFFECT_HEAL_BELL,
    EFFECT_MEAN_LOOK,
    EFFECT_NIGHTMARE,
...
bool32 MovesWithSplitUnusable(u32 attacker, u32 target, u32 split)
{
    ...
    return (usable == 0);
} //since set actually sets, may be able to use returnmovetype here?
//unsure if this is actually meant to set tpye or not
...
s32 AI_CalcDamage(u16 move, u8 battlerAtk, u8 battlerDef, u8 *typeEffectiveness, bool32 considerZPower)
{
   ...
   /* if (considerZPower && IsViableZMove(battlerAtk, move))
    ...
    else//if (gBattleMoves[move].power)
    {
        ...
        //    dmg *= 2;
        if (move == MOVE_SURGING_STRIKES || (move == MOVE_WATER_SHURIKEN && gBattleMons[battlerAtk].species == SPECIES_GRENINJA_ASH))
            dmg *= 3;
...
static u32 WhichMoveBetter(u32 move1, u32 move2)
{
    ...
    if (GetBattlerAbility(sBattler_AI) != ABILITY_ROCK_HEAD
        ...
        && GetBattlerAbility(sBattler_AI) != ABILITY_BONE_ARMOR)
    {
...
u32 GetNoOfHitsToKO(s32 dmg, s32 hp)
{
    if (dmg == 0)
        return 0;
    return hp / (dmg + 1) + 1;
...
bool32 AI_IsTerrainAffected(u8 battlerId, u32 flags)
{
    ...
    && flags == STATUS_FIELD_GRASSY_TERRAIN)
        return TRUE;
...
//because new floating setup is akin to natural levitate think would need,
//to set it up on odds like levitate was treated, where its not something,
//automatically known by all, say ace trainers would know,
//but average wouldn't know all, not sure how could do right now
//I feel like i need to guess w is it floating
//but if I do, then it causes issue of weakening ground type ai,
//as they wouldn't use ground moves when they could
//look at how flying check was handled and go from there vsonic IMPORTANT
bool32 AI_IsBattlerGrounded(u8 battlerId)
{
    ...
    return grounded;
}//vsonic adjust with my version in mind

//will need change further EE ai logic has expanded ex. aiFlags is an array
//as its used not for trainer but now reads specific flags for each battler,
//very useful
...
//believe will also have to update w new weather logic?
...
//completely reworked this all absorb redirect if not blocked by status,
//vsonic need update this
...
//this is a function I made, after redoing accuracy setup to use gettotalaccuracy for main function
//remove this and take emerald expansion setup which uses Ai_SetMoveAccuracy
//in battle_ai_main.c
...
//after rework dmg is based on species castform rather than forcast itself
//cant remember how I have form change setup,
//is castform staying the same species when it changes forms?
//may need to replace species check with base form species check...
//pretty sure I have that function in pokedex. done
...
bool32 ShouldSetHail(u8 battler, u16 ability, u16 holdEffect)
{
    ...
    return FALSE;
}//may adjust these for castform to also include held item check
...
//vsonic check make sure is fine
...
bool32 IsTrappingMoveEffect(u16 effect)
{
    ...
    case EFFECT_SNAP_TRAP:*/ //consolidated into just Effect Trap
...
//no reason  for this to be a switch, I'd want to add fly to this but it takes effect only
...
//vsonic add on to this and other below functions
//need to add poison legacy logic to this, well maybe not, it may pull what the counter is
static u32 GetPoisonDamage(u8 battlerId)
{
    u32 damage = 0;
    u8 turn = gBattleStruct->ToxicTurnCounter[gBattlerPartyIndexes[battlerId]][GetBattlerSide(battlerId)];
    ...
    else if (gBattleMons[battlerId].status1 & STATUS1_TOXIC_POISON)
    {
        ...
        if (damage == 0)
            damage = 1;
        if (turn != MAX_TOXIC_TURNS) // not 16 turns
...
static bool32 PartyBattlerShouldAvoidHazards(u8 currBattler, u8 switchBattler)
{
    ...
    || IsBattlerPrimalReverted(currBattler) || IsBattlerPrimalReverted(switchBattler))
...
bool32 AI_CanSleep(u8 battler, u16 ability)//why is comatose not here? vsonic had to add
{
    if (ability == ABILITY_INSOMNIA
      ...
      || AI_IsTerrainAffected(battler, STATUS_FIELD_ELECTRIC_TERRAIN)
...
static bool32 AI_CanBePoisoned(u8 battlerAtk, u8 battlerDef) //vsonic tweak with setmoveeffect logic
{
    ...
    if (!(AI_CanPoisonType(battlerAtk, battlerDef))
     ...
     || AI_IsAbilityOnSide(battlerDef, ABILITY_SHAMAN_CURE)
...
bool32 AI_CanPoison(u8 battlerAtk, u8 battlerDef, u16 defAbility, u16 move, u16 partnerMove)
{
    ...
    || IS_BATTLER_ANY_TYPE(battlerDef, TYPE_ROCK, TYPE_STEEL)))
        ...
        return FALSE;
    else if (IsValidDoubleBattle(battlerAtk) && AI_DATA->abilities[BATTLE_PARTNER(battlerDef)] == ABILITY_SHAMAN_CURE)
        return FALSE;
...
bool32 IsWakeupTurn(u8 battler)
{
    ...
    //if (gBattleStruct->SleepTimer[gBattlerPartyIndexes[battler]][GetBattlerSide(battler)] == 1 
    //&& FindMoveUsedXTurnsAgo(battler, 2) == MOVE_REST)
    if (gBattleStruct->SleepTimer[gBattlerPartyIndexes[battler]][GetBattlerSide(battler)] == 2
    && GetBattlerAbility(battler) == ABILITY_EARLY_BIRD) 
        return TRUE;
    else if (gBattleStruct->SleepTimer[gBattlerPartyIndexes[battler]][GetBattlerSide(battler)] == 1) 
        return TRUE;
    else // no way to know - changed, I THINK this should work???
        return FALSE;
}//change to use sleep turn //ok think this should work?
//ok so think my rework, makes this possible I can track the actual turns,
...
//no longer used took battle_util version, same logic other than type of argument
