...
static u32 GetAIEffectGroupFromMove(enum BattlerId battler, u32 move);
...
bool32 AI_IsFaster(enum BattlerId battlerAi, enum BattlerId battlerDef, u32 aiMove, u32 playerMove, enum ConsiderPriority considerPriority)
{
    return (AI_WhoStrikesFirst(battlerAi, battlerDef, aiMove, playerMove, considerPriority) == AI_IS_FASTER);
}

bool32 AI_IsSlower(enum BattlerId battlerAi, enum BattlerId battlerDef, u32 aiMove, u32 playerMove, enum ConsiderPriority considerPriority)
{
    return (AI_WhoStrikesFirst(battlerAi, battlerDef, aiMove, playerMove, considerPriority) == AI_IS_SLOWER);
}

u32 GetAIChosenMove(enum BattlerId battlerId)
{
...
u32 GetIncomingMove(enum BattlerId battler, u32 opposingBattler, struct AiLogicData *aiData)
{
...
u32 GetIncomingMoveSpeedCheck(enum BattlerId battler, u32 opposingBattler, struct AiLogicData *aiData)
{
    if (aiData->predictingMove && CanAiPredictMove(battler))
    {
...
void RecordKnownMove(enum BattlerId battler, u32 move)
{
    s32 moveIndex;
...
bool32 IsTruantMonVulnerable(enum BattlerId battlerAI, u32 opposingBattler)
{
...
struct SimulatedDamage AI_CalcDamageSaveBattlers(u32 move, enum BattlerId battlerAtk, enum BattlerId battlerDef, uq4_12_t *typeEffectiveness, enum AIConsiderGimmick considerGimmickAtk, enum AIConsiderGimmick considerGimmickDef)
{
...
bool32 IsAdditionalEffectBlocked(enum BattlerId battlerAtk, u32 abilityAtk, enum BattlerId battlerDef, u32 abilityDef)
{
...
struct SimulatedDamage AI_CalcDamage(u32 move, enum BattlerId battlerAtk, enum BattlerId battlerDef, uq4_12_t *typeEffectiveness, enum AIConsiderGimmick considerGimmickAtk, enum AIConsiderGimmick considerGimmickDef, u32 weather, u32 fieldStatuses)
{
...
static bool32 AI_IsMoveEffectInPlus(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move, s32 noOfHitsToKo)
{
...
static bool32 AI_IsMoveEffectInMinus(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move, s32 noOfHitsToKo)
{
    enum Ability abilityAtk = gAiLogicData->abilities[battlerAtk];
    enum Ability abilityDef = gAiLogicData->abilities[battlerDef];
...
enum MoveComparisonResult CompareMoveEffects(u32 move1, u32 move2, enum BattlerId battlerAtk, enum BattlerId battlerDef, s32 noOfHitsToKo)
{
...
uq4_12_t AI_GetMoveEffectiveness(u32 move, enum BattlerId battlerAtk, enum BattlerId battlerDef)
{
...
*/
s32 AI_WhoStrikesFirst(enum BattlerId battlerAI, enum BattlerId battler, u32 aiMoveConsidered, u32 playerMoveConsidered, enum ConsiderPriority considerPriority)
{
...
bool32 CanEndureHit(enum BattlerId battler, enum BattlerId battlerTarget, u32 move)
{
    ...
    if (!DoesBattlerIgnoreAbilityChecks(battler, gAiLogicData->abilities[battler], move))
    {
        ...
        if (gAiLogicData->abilities[battlerTarget] == ABILITY_ICE_FACE
            && gBattleMons[battlerTarget].species == GetBaseFormSpecies(gBattleMons[battlerTarget].species) && GetMoveCategory(move) == DAMAGE_CATEGORY_PHYSICAL)
            return TRUE;
    }
...
void GetBestDmgMovesFromBattler(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum DamageCalcContext calcContext, u32 *bestMoves)
{
    struct AiLogicData *aiData = gAiLogicData;
    u32 bestDmg = 0;
    u16 *moves = GetMovesArray(battlerAtk);
...
u16 GetMoveIndex(enum BattlerId battler, u32 move)
{
...
bool32 IsBestDmgMove(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum DamageCalcContext calcContext, u32 move)
{
...
bool32 CanTargetMoveFaintAi(u32 move, enum BattlerId battlerDef, enum BattlerId battlerAtk, u32 nHits)
{
...
bool32 DoesBattlerIgnoreAbilityChecks(enum BattlerId battlerAtk, enum Ability atkAbility, u32 move)
{
...
u32 AI_GetSwitchinWeather(enum BattlerId battler)
{
    ...
    case ABILITY_SNOW_WARNING:
        return GetConfig(B_SNOW_WARNING) >= GEN_9 ? WEATHER_SNOW : WEATHER_HAIL;
    default:
        return gBattleWeather;
    }
}
...
bool32 IsAllyProtectingFromMove(enum BattlerId battlerAtk, u32 attackerMove, u32 allyMove)
{
...
bool32 IsMoveRedirectionPrevented(enum BattlerId battlerAtk, u32 move, enum Ability atkAbility)
{
    if (gAiThinkingStruct->aiFlags[battlerAtk] & AI_FLAG_NEGATE_UNAWARE)
        return FALSE;
...
bool32 ShouldTryOHKO(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability atkAbility, enum Ability defAbility, u32 move)
{
...
bool32 ShouldRaiseAnyStat(enum BattlerId battlerAtk, enum BattlerId battlerDef)
{
    ...
    if (gBattleMons[battlerAtk].volatiles.yawn && CanBeSlept(battlerDef, battlerDef, gAiLogicData->abilities[battlerAtk], gAiLogicData->abilities[battlerDef], BLOCKED_BY_SLEEP_CLAUSE))
        return FALSE;
...
s32 ProtectChecks(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move, u32 predictedMove)
{
    s32 score = 0;
...
u32 IncreaseStatDownScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Stat stat)
{
...
u16 *GetMovesArray(enum BattlerId battler)
{
...
bool32 HasBattlerSideMoveWithEffect(enum BattlerId battler, u32 effect)
{
...
bool32 HasBattlerSideUsedMoveWithEffect(enum BattlerId battler, u32 effect)
{
...
bool32 HasNonVolatileMoveEffect(enum BattlerId battlerId, u32 effect)
{
    u16 *moves = GetMovesArray(battlerId);
...
bool32 HasMoveWithAdditionalEffect(enum BattlerId battlerId, u32 moveEffect)
{
    u16 *moves = GetMovesArray(battlerId);
...
bool32 HasBattlerSideMoveWithAdditionalEffect(enum BattlerId battler, u32 moveEffect)
{
...
bool32 HasMoveWithMoveEffectExcept(enum BattlerId battlerId, u32 moveEffect, enum BattleMoveEffects exception)
{
    u16 *moves = GetMovesArray(battlerId);
...
bool32 HasMove(enum BattlerId battlerId, u32 move)
{
    u16 *moves = GetMovesArray(battlerId);
...
bool32 IsTwoTurnNotSemiInvulnerableMove(enum BattlerId battlerAtk, u32 move)
{
    switch (GetMoveEffect(move))
    {
...
static u32 GetPoisonDamage(enum BattlerId battlerId)
{
    ...
    else if (gBattleMons[battlerId].status1 & STATUS1_TOXIC_POISON)
    {
        ...
        u8 ToxicTurnCounter = GetBattlerPartyState(battlerId)->ToxicTurnCounter;
        damage = gBattleMons[battlerId].maxHP / 16;
        if (damage == 0)
            damage = 1;
        if ((ToxicTurnCounter) != 15) // not 16 turns
            ToxicTurnCounter++;
        damage *= ToxicTurnCounter;
    }
    return damage;
}
...
enum AIPivot ShouldPivot(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move)
{
    u32 predictedMoveSpeedCheck = GetIncomingMoveSpeedCheck(battlerAtk, battlerDef, gAiLogicData);
    bool32 aiIsFaster = AI_IsFaster(battlerAtk, battlerDef, move, predictedMoveSpeedCheck, CONSIDER_PRIORITY);
...
bool32 CanKnockOffItem(u32 fromBattler, enum BattlerId battler, u32 item)
{
    if (item == ITEM_NONE)
        return FALSE;
...
bool32 AI_CanPutToSleep(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability defAbility, u32 move, u32 partnerMove)
{
...
bool32 ShouldBurn(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability abilityDef)
{
    ...
    if (CanBeBurned(battlerAtk, battlerDef, abilityDef) && (
        DoesBattlerBenefitFromAllVolatileStatus(battlerDef, abilityDef)
        || abilityDef == ABILITY_HEATPROOF || abilityDef == ABILITY_ASH_SPROUT
...
bool32 AI_CanPoison(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability defAbility, u32 move, u32 partnerMove)
{
...
bool32 AI_CanParalyze(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability defAbility, u32 move, u32 partnerMove)
{
...
bool32 AI_CanBeConfused(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move, enum Ability abilityDef)
{
...
bool32 AI_CanConfuse(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability defAbility, enum BattlerId battlerAtkPartner, u32 move, u32 partnerMove)
{
...
bool32 AI_CanBurn(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability defAbility, enum BattlerId battlerAtkPartner, u32 move, u32 partnerMove)
{
...
bool32 AI_CanGiveFrostbite(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability defAbility, enum BattlerId battlerAtkPartner, u32 move, u32 partnerMove)
{
...
u32 ShouldTryToFlinch(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability atkAbility, enum Ability defAbility, u32 move)
{
    u32 predictedMoveSpeedCheck = GetIncomingMoveSpeedCheck(battlerAtk, battlerDef, gAiLogicData);
...
bool32 ShouldTrap(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move)
{
    if (AI_CanBattlerEscape(battlerDef))
        return FALSE;
...
bool32 IsFlinchGuaranteed(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move)
{
...
static inline bool32 RecoveryEnablesWinning1v1(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move, u32 aiIsFaster, u32 healAmount)
{
    if (aiIsFaster)
    {
        if (CanTargetFaintAi(battlerDef, battlerAtk)
...
bool32 ShouldAbsorb(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move)
{
...
bool32 ShouldRecover(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move, u32 healPercent)
{
    u32 maxHP = gBattleMons[battlerAtk].maxHP;
    u32 currHP = gBattleMons[battlerAtk].hp;
...
bool32 ShouldSetScreen(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum BattleMoveEffects moveEffect)
{
    ...
    case EFFECT_AURORA_VEIL:
        ...
        if ((AI_GetWeather() & (WEATHER_COLD_ANY))
...
u32 GetAllyChosenMove(enum BattlerId battlerId)
{
...
// It matches both on move effect and on AI move effect; eg, EFFECT_HAZE will also bring up Freezy Frost or Clear Smog, anything with AI_EFFECT_RESET_STATS.
bool32 DoesPartnerHaveSameMoveEffect(enum BattlerId battlerAtkPartner, enum BattlerId battlerDef, u32 move, u32 partnerMove)
{
...
bool32 PartnerMoveEffectIsStatusSameTarget(enum BattlerId battlerAtkPartner, enum BattlerId battlerDef, u32 partnerMove)
{
    if (!HasPartner(battlerAtkPartner))
        return FALSE;
...
bool32 PartnerMoveEffectIs(enum BattlerId battlerAtkPartner, u32 partnerMove, enum BattleMoveEffects effectCheck)
{
    if (!HasPartner(battlerAtkPartner))
        return FALSE;
...
bool32 PartnerMoveIs(enum BattlerId battlerAtkPartner, u32 partnerMove, u32 moveCheck)
{
    if (!HasPartner(battlerAtkPartner))
        return FALSE;
...
bool32 PartnerMoveIsSameAsAttacker(enum BattlerId battlerAtkPartner, enum BattlerId battlerDef, u32 move, u32 partnerMove)
{
    if (!HasPartner(battlerAtkPartner))
        return FALSE;
...
bool32 PartnerMoveIsSameNoTarget(enum BattlerId battlerAtkPartner, u32 move, u32 partnerMove)
{
    if (!HasPartner(battlerAtkPartner))
        return FALSE;
...
bool32 ShouldUseWishAromatherapy(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move)
{
    s32 firstId, lastId;
    struct Pokemon* party;
...
bool32 PartyHasMoveCategory(enum BattlerId battlerId, enum DamageCategory category)
{
    ...
    for (u32 monIndex = 0; monIndex < PARTY_SIZE; monIndex++)
    {
        ...
        for (u32 moveIndex = 0; moveIndex < MAX_MON_MOVES; moveIndex++)
        {
            u32 move = GetMonData(&party[monIndex], MON_DATA_MOVE1 + moveIndex);
...
bool32 ShouldRestoreHpBerry(enum BattlerId battlerAtk, u32 item)
{
    switch (item)
    {
...
bool32 HasHPForDamagingSetup(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 hpThreshold)
{
    ...
    //change drops hp by 1 on break w second hit
    //not big deal but may update this later
    if (bestMoveIsPhysical
     && gAiLogicData->abilities[battlerAtk] == ABILITY_ICE_FACE
     && gBattleMons[battlerAtk].species == GetBaseFormSpecies(gBattleMons[battlerAtk].species)
...
u32 IncreaseStatUpScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum StatChange statChange)
{
    return IncreaseStatUpScoreInternal(battlerAtk, battlerDef, statChange, TRUE);
}

u32 IncreaseStatUpScoreContrary(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum StatChange statChange)
{
    return IncreaseStatUpScoreInternal(battlerAtk, battlerDef, statChange, FALSE);
}

void IncreasePoisonScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move, s32 *score)
{
...
void IncreaseBurnScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move, s32 *score)
{
    if (((gAiThinkingStruct->aiFlags[battlerAtk] & AI_FLAG_TRY_TO_FAINT) && CanAIFaintTarget(battlerAtk, battlerDef, 0))
...
void IncreaseParalyzeScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move, s32 *score)
{
    if (((gAiThinkingStruct->aiFlags[battlerAtk] & AI_FLAG_TRY_TO_FAINT) && CanAIFaintTarget(battlerAtk, battlerDef, 0))
...
void IncreaseSleepScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move, s32 *score)
{
...
void IncreaseConfusionScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move, s32 *score)
{
    if (((gAiThinkingStruct->aiFlags[battlerAtk] & AI_FLAG_TRY_TO_FAINT) && CanAIFaintTarget(battlerAtk, battlerDef, 0))
...
void IncreaseFrostbiteScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move, s32 *score)
{
    if ((gAiThinkingStruct->aiFlags[battlerAtk] & AI_FLAG_TRY_TO_FAINT) && CanAIFaintTarget(battlerAtk, battlerDef, 0))
        return;
...
bool32 ShouldUseZMove(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 chosenMove)
{
    ...
    if (IsViableZMove(battlerAtk, chosenMove))
    {
        ...
        if (gBattleMons[battlerDef].ability == ABILITY_ICE_FACE
            && !MoveIgnoresTargetAbility(zMove)
            && gBattleMons[battlerDef].species == GetBaseFormSpecies(gBattleMons[battlerDef].species) && IsBattleMovePhysical(chosenMove))
...
bool32 AI_ShouldSetUpHazards(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move, struct AiLogicData *aiData)
{
...
void IncreaseTidyUpScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move, s32 *score)
{
...
bool32 AI_ShouldSpicyExtract(enum BattlerId battlerAtk, enum BattlerId battlerAtkPartner, u32 move, struct AiLogicData *aiData)
{
...
u32 IncreaseSubstituteMoveScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move)
{
    enum BattleMoveEffects effect = GetMoveEffect(move);
    u32 scoreIncrease = 0;
...
bool32 CanEffectChangeAbility(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move, struct AiLogicData *aiData)
{
    enum BattleMoveEffects effect = GetMoveEffect(move);
...
void AbilityChangeScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move, s32 *score, struct AiLogicData *aiData)
{
    enum BattleMoveEffects effect = GetMoveEffect(move);
...
bool32 ShouldConsiderSelfSacrificeDamageEffect(enum BattlerId battlerAtk, enum BattlerId battlerDef, u32 move, bool32 aiIsFaster)
{
...
u32 GetActiveBattlerIds(enum BattlerId battler, enum BattlerId *battlerIn1, enum BattlerId *battlerIn2)
{
    ...
    enum BattlerId battlerPosition = GetBattlerPosition(battler);
    if (IsDoubleBattle())
    {
        *battlerIn1 = battler;
