...
//no idea what effect this should have - vsonic
...
*/
//ee update script fle and effects nother major thing
//to look into
...
//equiv of pause and waitmessage, used in overworld scripts
//and battle anims, if use if (gMain.inBattle)
//to exclude the overworld scripts, think can do
//same thing I did in other, I think,
//would be helpful for setup with pokeabbies speed up stuff

//since this is used in many places rather than change value
//and replace w array, think instead will have to just divide by 2
//or reduce the value by a flat amount only if in battle
//to ensure this isn't affecting ow scripts, can test with new game oak grass script
//ok now I see the issue, the delay used in events and the delay used in battle anims
//are two different things their script commands are in battle_anim.c
#define SCRIPT_DELAY
...
bool8 ScrCmd_messageautoscroll(struct ScriptContext * ctx)
{
    ...
    //gTextFlags.forceMidTextSpeed = TRUE; //added from emerald not sure if want this way
    ShowFieldAutoScrollMessage(msg); //currenlty cmd only used incable club so never seen in game
    return FALSE;
}
...
//idea and code from zatsu replace for individual data call,
//2 argument pick data to retrieve and what mon slot in party to check
//set see if actually works
//follow with compare result var, 
//compare VAR_RESULT, TRUE/FALSE
...
//this is source of getspeciesname macro found in event.inc
...
//ok so way it works is these are macros,
//logic is set here, function name is added to script_cmd_table.inc
//then to use it in scripts a reference name is made in event.inc
//order (byte count) must match with order in script_cmd_table.inc
//difference between special and macro I guess is macros can't return things?
//which is why these all return 0, and the return value isn't used by the scripts
//just the data put in the buffer
...
const u8 gUnknown_83A72A0[] = _("s"); //decapped so can be capped in sync with item cap
...
bool8 ScrCmd_bufferitemnameplural(struct ScriptContext * ctx)
{
    ...
    if (itemId == ITEM_POKE_BALL && quantity >= 2)
    {
        if (ShouldCapitalizeItems())
            StringCopy(gStringVar3, gUnknown_Cap83A72A0);
        else
            StringCopy(gStringVar3, gUnknown_83A72A0);
    ...
    else if (GetPocketByItemId(itemId) == POCKET_BERRIES && quantity >= 2)
    {
        ...
        if (strlength != 0)
        {
            ...
            endptr[-1] = EOS;
            if (ShouldCapitalizeItems())
                StringCopy(gStringVar3, gUnknown_Cap83A72A2);
            else
                StringCopy(gStringVar3, gUnknown_83A72A2);
            //CapializeString(gUnknown_83A72A2); 
            StringAppend(sScriptStringVars[stringVarIndex], gStringVar3);
        }
    }
...
bool8 ScrCmd_buffermovename(struct ScriptContext * ctx)
{
    ...
    GetMoveName(sScriptStringVars[stringVarIndex], moveId);
    return FALSE;
}
...
//this is the one that has the level scaling
...
//I copied the base logic and put it in this second one
//just for specific things that should stay level 5
bool8 ScrCmd_givemon2(struct ScriptContext * ctx)
{
    u16 species = VarGet(ScriptReadHalfword(ctx));
    u8 level = ScriptReadByte(ctx);
    enum Item item = VarGet(ScriptReadHalfword(ctx));
...
//works
bool8 ScrCmd_givegroup(struct ScriptContext * ctx)
{
    u32 i;
    u8 loops = ScriptReadByte(ctx);
...
bool8 ScrCmd_UpdateOakRanch(void) //to run oak ranch update from cmd
{
...
//in scripts all lists ended with ITEM_NONE
//since in forest, think will have viridian
//sell beries, maybe a leek as well?
static const u16 sMartItems_0Badges[] =
{
    ITEM_POKE_BALL,
    ITEM_POTION,
    ITEM_ANTIDOTE,
...
static const u16  sMartItems_1Badges[] =
{
    ITEM_POKE_BALL,
    ITEM_SUPER_POTION,
    ITEM_POTION,
...
static const u16  sMartItems_2Badges[] =
{
    ITEM_POKE_BALL,
    ITEM_SUPER_POTION,
    ITEM_POTION,
...
static const u16  sMartItems_3Badges[] =
{
    ITEM_GREAT_BALL,
    ITEM_POKE_BALL,
    ITEM_SUPER_POTION,
...
static const u16  sMartItems_4Badges[] =
{
    ITEM_GREAT_BALL,
    ITEM_POKE_BALL,
    ITEM_SUPER_POTION,
...
static const u16  sMartItems_5Badges[] =
{
    ITEM_ULTRA_BALL,
    ITEM_GREAT_BALL,
    ITEM_POKE_BALL,
...
static const u16  sMartItems_6Badges[] =
{
    ITEM_ULTRA_BALL,
    ITEM_GREAT_BALL,
    ITEM_POKE_BALL,
...
static const u16  sMartItems_7Badges[] =
{
    ITEM_ULTRA_BALL,
    ITEM_GREAT_BALL,
    ITEM_POKE_BALL,
...
static const u16  sMartItems_8Badges[] =
{
    ITEM_ULTRA_BALL,
    ITEM_GREAT_BALL,
    ITEM_POKE_BALL,
...
//use +1 as need value for 0 badges as well
const u16 *const gMartItemIndexesbyGymBadge[NUM_BADGES + 1] =
{
    sMartItems_0Badges,
    sMartItems_1Badges,
    sMartItems_2Badges,
...
//may potentially move these to new game?
//just seems to not really fit here?

//reserve this for 2nd clerk, and potentially celadon
//otherwise make copy of this without ptr logic
//instead add badge count check and switch to relevant one of gmartitems

// found PrintBadgeCount could have taken code from,
//but like my verison better, believe more efficient,
//...oh ok nvm mind stops as soon as you don't have a badge,
//but point is to be able to get badges out of order so need to check all badge flags
u8 GetNumBadges(void) //w changes should work now, pretty much identical to trainer_card function anyway
{
    u32 flagId;
    u8 NumBadges = 0;
...
//WORKS!!!
//builds list without issue, just need to setup badge check
//and fill in other functions!!
bool8 ScrCmd_pokemart2(void)
{
    //const void *ptr = (void *)ScriptReadWord(ctx);
