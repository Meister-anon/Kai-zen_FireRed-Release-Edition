...
//used with sWildEncounterData.abilityEffect
enum EncounterAbilityEffects
{
    NO_EFFECT_ON_RATE,
...
{
    sWildEncounterData.abilityEffect = NO_EFFECT_ON_RATE;
    if (!GetMonData(&gPlayerParty[0], MON_DATA_SANITY_IS_EGG))
    {
...
bool8 UpdateRepelCounter(void)
{
    ...
    if (steps != 0)
    {
        ...
        if (steps == 0)
        {
            ScriptContext_SetupScript(EventScript_BwRepelWoreOff); //missed adding this back in port
            return TRUE;
        }
    }
    return FALSE;
