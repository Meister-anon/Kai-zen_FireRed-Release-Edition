...
s32 GetSpeciesPokedexAreaMarkers(u16 species, struct Subsprite * subsprites)
{
    ...
    for (i = 0, areaCount = 0; gWildMonHeaders[i].mapGroup != 0xFF; i++)
    {
        ...
        if (IsSpeciesOnMap(&gWildMonHeaders[i], species))
        {
            ...
            tableIndex = 0;
            while (TryGetMapSecPokedexAreaEntry(mapSecId, sDexAreas_Kanto, 55, &tableIndex, &dexAreaSubspriteIdx))
            {
                if (dexAreaSubspriteIdx != DEX_AREA_NONE)
                {
                    GetAreaMarkerSubsprite(areaCount++, dexAreaSubspriteIdx, subsprites);
                }
            }
            for (j = 0; j < NELEMS(sSeviiDexAreas); j++)
            {
                if ((seviiAreas >> j) & 1)
                {
                    ...
                    tableIndex = 0;
                    while (TryGetMapSecPokedexAreaEntry(mapSecId, sSeviiDexAreas[j].table, sSeviiDexAreas[j].count, &tableIndex, &dexAreaSubspriteIdx))
                    {
                        if (dexAreaSubspriteIdx != DEX_AREA_NONE)
                        {
                            GetAreaMarkerSubsprite(areaCount++, dexAreaSubspriteIdx, subsprites);
...
static s32 GetRoamerPokedexAreaMarkers(u16 species, struct Subsprite * subsprites)
{
    ...
    tableIndex = 0;
    if (TryGetMapSecPokedexAreaEntry(roamerLocation, sDexAreas_Kanto, 55, &tableIndex, &dexAreaSubspriteIdx))
    {
        if (dexAreaSubspriteIdx != 0)
        {
            GetAreaMarkerSubsprite(0, dexAreaSubspriteIdx, subsprites);
            return 1;
        }
    }
    return 0;
