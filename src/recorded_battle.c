...
//mostly default go over later and remove what I can, mostly sure can remove palace stuff
            ...
            else // B_RECORD_MODE_PLAYBACK
            {
                if (sBattleRecords[battlerId][sBattlerRecordSizes[battlerId]] == ACTION_MOVE_CHANGE)
                {
                    ...
                    for (j = 0; j < MAX_MON_MOVES; j++)
                    {
                        ...
                        mimickedMoveSlots[j] = (gBattleMons[battlerId].volatiles.mimickedMoves & (1u << j)) >> j;
                    }
                    for (j = 0; j < MAX_MON_MOVES; j++)
                    {
                    ...
                    gBattleMons[battlerId].volatiles.mimickedMoves = 0;
                    for (j = 0; j < MAX_MON_MOVES; j++)
                    {
