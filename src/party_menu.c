...
static void CreateCancelConfirmPokeballSprites(void)
{
    ...
    else if (gPartyMenu.action == PARTY_ACTION_SEND_OUT) //attempt
    {
        // The showcase has no Cancel/Confirm buttons
        FillBgTilemapBufferRect(1, 14, 23, 17, 7, 2, 1);
    } //works just need remove text as well.
    else
    {
...
static void HandleChooseMonCancel(u8 taskId, s8 *slotPtr)
{
    switch (gPartyMenu.action)
    {
    case PARTY_ACTION_SEND_OUT: //see if can use this to trigger create menu without cancel icon
        PlaySE(SE_FAILURE);
        break;
    case PARTY_ACTION_SWITCH:
    case PARTY_ACTION_SOFTBOILED:
...
static void UpdatePartySelectionSingleLayout(s8 *slotPtr, s8 movementDir)
{
    ...
    case MENU_DIR_UP:
        ...
        break;
    case MENU_DIR_DOWN:
        if (*slotPtr == PARTY_SIZE + 1 && (gPartyMenu.action != PARTY_ACTION_SEND_OUT)) //partysize + 1 is already excepted from this, but just to make it more clear
        {
            *slotPtr = 0;
        }
        else
        {
            if (*slotPtr == gPlayerPartyCount - 1)
            {
                ...
                {  
                    if (gPartyMenu.action != PARTY_ACTION_SEND_OUT)
                        *slotPtr = PARTY_SIZE + 1;//place I want may be here instead?
                    else
                        *slotPtr = 0;
                }
            }
...
static void UpdatePartySelectionDoubleLayout(s8 *slotPtr, s8 movementDir)
{
    ...
    case MENU_DIR_DOWN:
        ...
        else
        {
            ...
            if (newSlot == -1) //slot + 1 is greater or equal party size
            {
                if (gPartyMenu.action != PARTY_ACTION_SEND_OUT)
                {
                    if (sPartyMenuInternal->chooseHalf)
                    ...
                    *slotPtr = 0; //still not sure if right but will try
            } //singles works need test double //double works seems fine
            else
            {
                *slotPtr = newSlot;
            }
...
u8 *GetMonNickname(struct Pokemon *mon, u8 *dest)
{
    ...
    if (StringCompare(gSpeciesNames[species], dest) == IDENTICAL) //if not nicknamed reassign tempStr to speciesname, making it update capitalization
...
static void DisplayGaveHeldItemMessage(struct Pokemon *mon, u16 item, bool8 keepOpen, bool8 fromBagMenu)
{
    ...
    if (StringCompare(gSpeciesNames[GetMonData(mon,MON_DATA_SPECIES)], gStringVar1) == IDENTICAL) /*if not nicknamed reassign tempStr to speciesname, making it update capitalization*/\
...
static void CreateCancelConfirmWindows(bool8 chooseHalf)
{
    ...
    if (gPartyMenu.menuType != PARTY_MENU_TYPE_MULTI_SHOWCASE && (gPartyMenu.action != PARTY_ACTION_SEND_OUT))
    {
        if (chooseHalf == TRUE)
        {
...
static void DisplayPartyPokemonNickname(struct Pokemon *mon, struct PartyMenuBox *menuBox, u8 c)
{
    ...
    if (species != SPECIES_NONE)
    {
        ...
        if (StringCompare(gSpeciesNames[species], nickname) == IDENTICAL) /*if not nicknamed reassign tempStr to speciesname, making it update capitalization*/\
...
} //pretty sure these line up with the party_menu.h arrays from sPartyBoxInfoRects
//value 4,5,6,7 lines up wiith the level field,  and fr gender symbol below 8,9,10,11 lines up with that..
//no idea why it worked but fixed graphic issue by adding +1 to x and width values
...
static void DisplayPartyPokemonGender(u8 gender, u16 species, u8 *nickname, struct PartyMenuBox *menuBox)
{
    ...
    if (species == SPECIES_NONE)
        return;
    if ((species == SPECIES_NIDORAN_M || species == SPECIES_NIDORAN_F) && StringCompare(nickname, gSpeciesNames[species]) == IDENTICAL)
        return;
    switch (gender)
    {
    case MON_MALE:
...
//holy shit I DID IT!!!
//this actually works, consider if can use for dex,
//repalced pointer to const u8 *text with just variable value u8 Id and got this to work
//use this function to pas what const value back in, using global string buffer to pass and store
...
static const u8* ReturnCursorIdText(u8 i)
{
    ...
    switch(sCursorOptions[sPartyMenuInternal->actions[i]].Id)
    {
        case MENU_SUMMARY:
            return gText_Summary5;
        case MENU_SWITCH:
        ...
        case MENU_SHIFT:
            return gText_Shift;
        case MENU_SEND_OUT:
            return gText_SendOut;
        case MENU_ENTER:
            return gText_Enter;
        case MENU_NO_ENTRY:
            return gText_NoEntry;
        case MENU_STORE:
            return gText_Store;
        case MENU_REGISTER:
            return gText_Register;
        case MENU_TRADE1:
            return gText_Trade4;
        case MENU_TRADE2:
            return gText_Trade4;
        case (MENU_FIELD_MOVES + FIELD_MOVE_CUT):
            GetMoveName(gStringVar4, MOVE_CUT);
                return gStringVar4;
        case (MENU_FIELD_MOVES + FIELD_MOVE_FLY):
            GetMoveName(gStringVar4, MOVE_FLY);
                return gStringVar4;
        case (MENU_FIELD_MOVES + FIELD_MOVE_SURF):
            GetMoveName(gStringVar4, MOVE_SURF);
                return gStringVar4;
        case (MENU_FIELD_MOVES + FIELD_MOVE_STRENGTH):
            GetMoveName(gStringVar4, MOVE_STRENGTH);
                return gStringVar4;
        case (MENU_FIELD_MOVES + FIELD_MOVE_FLASH):
            GetMoveName(gStringVar4, MOVE_FLASH);
                return gStringVar4;
        case (MENU_FIELD_MOVES + FIELD_MOVE_ROCK_SMASH):
            GetMoveName(gStringVar4, MOVE_ROCK_SMASH);
                return gStringVar4;
        case (MENU_FIELD_MOVES + FIELD_MOVE_WATERFALL):
            GetMoveName(gStringVar4, MOVE_WATERFALL);
                return gStringVar4;
        ...
        case (MENU_FIELD_MOVES + FIELD_MOVE_TELEPORT):
            GetMoveName(gStringVar4, MOVE_TELEPORT);
                return gStringVar4; 
        case (MENU_FIELD_MOVES + FIELD_MOVE_DIG):
            GetMoveName(gStringVar4, MOVE_DIG);
                return gStringVar4;
        case (MENU_FIELD_MOVES + FIELD_MOVE_MILK_DRINK):
            GetMoveName(gStringVar4, MOVE_MILK_DRINK);
                return gStringVar4;
        case (MENU_FIELD_MOVES + FIELD_MOVE_SOFT_BOILED):
            GetMoveName(gStringVar4, MOVE_SOFT_BOILED);
                return gStringVar4;
...
static u8 DisplaySelectionWindow(u8 windowType)
{
    ...
    for (i = 0; i < sPartyMenuInternal->numActions; ++i)
    {
        ...
        AddTextPrinterParameterized4(sPartyMenuInternal->windowId[0], 2, cursorDimension, (i * 16) + 2, fontAttribute, 0, sFontColorTable[fontColorsId], 0, ReturnCursorIdText(i));
...
static void PartyMenu_Oak_PrintText(u8 windowId, const u8 *str)
{
    ...
    gTextFlags.canABSpeedUpPrint = TRUE;
    AddTextPrinterParameterized2(windowId, 4, gStringVar4, GetPlayerTextSpeedDelay(), NULL, 2, 1, 3);
}
...
{
   //look into AddItemIconObjectWithCustomObjectTemplate for this
...
static void GetMedicineItemEffectMessage(u16 item)
{
    ...
    case ITEM_EFFECT_HEAL_PP:
        ...
        break;
    case ITEM_EFFECT_SET_FORM:
        StringExpandPlaceholders(gStringVar4, gText_SetFormFlag);
        break;
    default:
...
static void Task_DisplayHPRestoredMessage(u8 taskId)
{
    ...
    if (StringCompare(gSpeciesNames[GetMonData(&gPlayerParty[gPartyMenu.slotId],MON_DATA_SPECIES)], gStringVar1) == IDENTICAL) /*if not nicknamed reassign tempStr to speciesname, making it update capitalization*/\
...
void Task_AbilityCapsule(u8 taskId) //important seemed easy enough so ported now, also ftw you can defnie text anywhere FP,
{
    ...
    case 0:
        ...
        if (StringCompare(gSpeciesNames[GetMonData(&gPlayerParty[tMonId],MON_DATA_SPECIES)], gStringVar1) == IDENTICAL) /*if not nicknamed reassign tempStr to speciesname, making it update capitalization*/\
...
u8 GetItemEffectType(u16 item)
{
    ...
    //but this works fine o.0
    //-...pretty sure this line here is pointless oak stops you would never get passed use

    //hmm you know what that actually would work,
    //because they have oak stops and null on every item that
    //isn't usable  I shold be able to use those as condition
    //rather than item range, and it won't change
    //the way they work in menu AT ALL
    ...
    if (itemEffect[1] & ITEM1_SET_FORM)
        return ITEM_EFFECT_SET_FORM;
...
static bool8 TrySwitchInPokemon(void)
{
    ...
    if (GetMonData(&gPlayerParty[slot], MON_DATA_HP) == 0)
    {
        ...
        if (StringCompare(gSpeciesNames[GetMonData(&gPlayerParty[slot],MON_DATA_SPECIES)], gStringVar1) == IDENTICAL) /*if not nicknamed reassign tempStr to speciesname, making it update capitalization*/\
    ...
    if (gPartyMenu.action == PARTY_ACTION_CANT_SWITCH)
    {
        ...
        if (StringCompare(gSpeciesNames[GetMonData(&gPlayerParty[GetPartyIdFromBattlePartyId(gBattlerPartyIndexes[currBattler])],MON_DATA_SPECIES)], gStringVar1) == IDENTICAL) /*if not nicknamed reassign tempStr to speciesname, making it update capitalization*/\
