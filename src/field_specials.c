...
bool8 AreLeadMonEVsMaxedOut(void)
{
    ...
    if (GetMonEVCount(&gPlayerParty[leadMonIndex]) >= MAX_TOTAL_EVS)
        return TRUE;
    else if ((GetBaseStatTotal(species) + (GetMonEVCount(&gPlayerParty[leadMonIndex]) / 4)) >= Global_Stat_Total_Limit)
        return TRUE;
    else
        return FALSE;
}
...
void SampleResortGorgeousMonAndReward(void)
{
    ...
    GetSpeciesName(gStringVar1, VarGet(VAR_RESORT_GORGEOUS_REQUESTED_MON));
}
...
//readjusting early learnsets of starter choice,
//so better reflects typical starter scaling
//i.e learns  move at 6, usually stab - note gen 1 mon didn't get a move till 7 so not hard rule
//must have an attack move by lvl 5
...
u16 GetFinalEvo(u16 species) //well least it works now
{
    ...
    }//new logic froze game, think issue was new null defers to species none
...
}//keep an eye out for any more issues, ok had to fix species none evo like EE now it works
...
u16 De_Evolve(u16 targetSpecies)
{
    ...
    for (species = 0; species != FORMS_START; species++)
    {
        ...
        NUM_EVOS_CAP = gSpeciesInfo[SanitizeSpeciesId(species)].evolutions == NULL ? EVOS_PER_MON : EVOLUTIONS_END;
...
{
    u8 passedChecks = 0;
    u8 Atktype1 = gSpeciesInfo[Atk_Species].type1;
    u8 Atktype2 = gSpeciesInfo[Atk_Species].type2;
    u8 targetType1 = gSpeciesInfo[Target_Species].type1;
    u8 targetType2 = gSpeciesInfo[Target_Species].type2;
...
bool8 NameRaterWasNicknameChanged(void)
{
    ...
    GetMonData(pokemon, MON_DATA_NICKNAME, gStringVar1);
    if (StringCompare(gStringVar3, gStringVar1) == IDENTICAL)
        return FALSE;
    else
        return TRUE;
}
