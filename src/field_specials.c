...
u8 GetLeadMonIndex(void)
{
    ...
    {
        pokemon = &gPlayerParty[i];
        if (GetMonData(pokemon, MON_DATA_SPECIES_OR_EGG) != SPECIES_EGG && GetMonData(pokemon, MON_DATA_SPECIES_OR_EGG) != SPECIES_NONE)
            return i;
    }
    return 0;
}
...
static bool8 HasMonBeenRenamed(u8 idx)
{
    ...
    if (language != LANGUAGE_ENGLISH)
        return TRUE;
    else if (StringCompare(gBaseStats[GetMonData(pokemon, MON_DATA_SPECIES)].speciesName, gStringVar1) != IDENTICAL)
        return TRUE;
    else
        return FALSE;
}
