...
static const struct SinglesTrainerInfo sSingleBattleTrainerInfo[] = {
    ...
    {OBJ_EVENT_GFX_OLD_MAN_1,      FACILITY_CLASS_GAMBLER,            MALE},
...
static const struct TrainerEncounterMusicPairs sTrainerEncounterMusicLUT[105] = {
    ...
    {FACILITY_CLASS_GAMBLER,            TRAINER_ENCOUNTER_MUSIC_FEMALE},
...
static void SetUpTrainerTowerDataStruct(void) // fakematching
{
    ...
    else
    {
        ...
        for (i = 0; i < MAX_TRAINER_TOWER_FLOORS; i++)
        {
            ...
            r5 = (void *)(i * sizeof(struct TrainerTowerFloor));
            r0  = r5 + (uintptr_t)r0;
            r0 += offsetof(struct TrainerTowerState, data.floors);
            memcpy(r0, floors_p[i], sizeof(struct TrainerTowerFloor));
        #else
            memcpy(&sTrainerTowerState->data.floors[i], floors_p[i], sizeof(struct TrainerTowerFloor));
        #endif
        }
...
static s32 GetPartyMaxLevel(void)
{
    ...
    {
        if (GetMonData(&gPlayerParty[i], MON_DATA_SPECIES) != 0 && GetMonData(&gPlayerParty[i], MON_DATA_SPECIES_OR_EGG) != SPECIES_EGG)
        {
            s32 currLevel = GetMonData(&gPlayerParty[i], MON_DATA_LEVEL);
