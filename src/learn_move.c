...
static void MoveRelearnerInitListMenuBuffersEtc(void)
{
    ...
    if (StringCompare(gSpeciesInfo[GetMonData(&gPlayerParty[sMoveRelearner->selectedPartyMember], MON_DATA_SPECIES)].speciesName, nickname) == IDENTICAL) //if not nicknamed reassign tempStr to speciesname, making it update capitalization
...
{
    u8 buffer[50];
    BlitMoveInfoIcon(2, gMovesInfo[move].type + 1, 1, 4);
