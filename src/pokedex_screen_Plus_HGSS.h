...
// u16 ignored is passed but never used
//this is for dex scrolling mon pic
...
static void Task_LoadEvolutionScreen(u8 taskId)
{
    ...
    //Print evo info and icons
    {
        ...
        if (sPokedexView->sEvoScreenData.numAllEvolutions != 0)// && sPokedexView->sEvoScreenData.numSeen != 0) //this is part that prevents arrow from showing for only 1 evo/mon seen
        {
...
static void Task_HandleEvolutionScreenInput(u8 taskId)
{
   ...
   // if (sPokedexView->sEvoScreenData.numAllEvolutions != 0)// && sPokedexView->sEvoScreenData.numSeen != 0)
    //{
        ...
        if (sPokedexView->sEvoScreenData.numAllEvolutions != 0)
        {
            if (JOY_NEW(DPAD_DOWN)) //dpad on evo doesn't work right, moving up works, but moving down only works when already down
            {
                ...
                if (sPokedexView->sEvoScreenData.menuPos == sPokedexView->sEvoScreenData.numAllEvolutions - 1)
                    sPokedexView->sEvoScreenData.menuPos = 0;// = sPokedexView->sEvoScreenData.numAllEvolutions - 1;
                else
                ...
                //sPokedexView->sEvoScreenData.menuPos = pos; //store end position
            }
        ...
        if (!(VarGet(VAR_0x408C)))
            VarSet(VAR_0x408C, NationalPokedexNumToSpeciesHGSS(sPokedexListItem->dexNum));
        ...
        if (JOY_NEW(A_BUTTON))
        {
            ...
            if (sPokedexView->sEvoScreenData.seen[sPokedexView->sEvoScreenData.menuPos] == FALSE)
                PlaySE(SE_FAILURE);
            else
            {
    ...
    {   //if not on initial species page you loaded into dex with, load that page
        if (VarGet(VAR_0x408C) != NationalPokedexNumToSpeciesHGSS(sPokedexListItem->dexNum))
        {
...
//use hasDuplicateEvos as bool, and storage value
//if 0 has no duplicates evo methods so tret as normal,
//otherwise print things differently
static void HandleTargetSpeciesPrint(u8 taskId, u16 NexttargetSpecies, u16 currentTargetSpecies, u8 base_x, u8 base_y, u8 base_y_offset, u8 base_i, u8 hasDuplicateEvos, bool8 isEevee)
{
    u8 iterations = 6;
    bool8 seen = GetSetPokedexFlag(SpeciesToNationalPokedexNum(currentTargetSpecies), FLAG_GET_SEEN);

    if (seen || !HGSS_HIDE_UNSEEN_EVOLUTION_NAMES)
        StringCopy(gStringVar3, GetSpeciesName(currentTargetSpecies)); //evolution mon name
    else
    ...
    //wondering how this works, as seems to only be able to display max 6 iterations in window, based on torchic set, 
    //holds base species, and then 6 icons 
    //rather than being static could make iterations a function of itmes, and if iteration is greater than 6 just do the eevee spacing
    if (isEevee)
    {
        iterations = 9;
        if (NexttargetSpecies == currentTargetSpecies) //checked eevee and it doesn't repeat list, icons dispite having glaceon and leafeon who have dupes
            return;
        else if (currentTargetSpecies == SPECIES_LEAFEON)
            base_i -= 1;    //what this does is fold the icon position onto another preceeding one. essentially making it LOOK like it printed once
        else if (currentTargetSpecies == SPECIES_GLACEON)
            base_i -= 2;
        else if (currentTargetSpecies == SPECIES_SYLVEON)
            base_i -= 2;
        /*else if (currentTargetSpecies == SPECIES_GLACEON)
            base_i -= 1;    //what this does is fold the icon position onto another preceeding one. essentially making it LOOK like it printed once
        else if (currentTargetSpecies == SPECIES_SYLVEON)
    ...
    else if (hasDuplicateEvos) //actually think issue was this was if, so not exclusive from eevee logic, eh if that was the case either way this should help
    {
        if (NexttargetSpecies == currentTargetSpecies)
            return;
        else
            base_i -= 1; //works for first evo but not 2nd, think would need to separate into stages
            //if curr target is a stage 1 evo, use -1, if 2nd stage evo use -2, but idk how...
    }
    //figured it out from eevees, its not based on evo stage, its that the spaces on the bar get filled regardless of if it fits in the window
    //and the amount of space/the number of slots is determined by below, the dimmensoins multiplied into base_i, smaller number more slots
    //but whether you need -1 or -2 is the determined by the mon's "slot" in relation to its spot in reference to a duplicate entry
    //in leafeons case it was the duplicate and the first, so there were no duplicates / just 1 extra space before it, so it only needed -1
    //glaceon was also a duplicate so it had one extra space before it, but because its after leafeon also a duplicate there are 2 spaces to cross
    //finally sylveon not a duplicate but behind 2 duplicates, so it had to cover 2 spaces
    
    //so translating to to torchic,  combuskin needs minus 1, because its the first duplicate, 
    //while blaziken would need to cover 2 spaces as its the 2nd duplicate.
    //I'm not 100% but I believe for every duplicate species I add to my list I would need to subtract an extra value from base_i to make it fit?
    //can test by making sylveon a duplicate entry, but I believe also it kinda depend on the number of evo methods for said species.
    //in this case so far it looks like (num evo mehods - 1) potentially + 1 for every duplicate before you in the list/order

    //tested by making sylveon also a duplicate and was able to confirm that is the case,
    //2 evo methods for sylveon so num mehtods -1 is 1,  but there are 2 duplicate mon before it
    //so adding that to my figures sylveon needs base_i -= 3  to be seen/fit
    
    //so what I need is, num evo methods for this target species  (subtract 1 from that value)
    //the number of mon on the list that are duplicate evos, which I believe I already have? 
    //whether current species is a duplicate, previous species is a duplicate, and next species is a duplicate (accounting for max 3 alt evo methods for a species)
    //but what I actually need is how many duplicates were before current species in list, and if current is a duplicate (+1 for each preceeding dupe)

    //also should be able to rework iterations to use evos per mon
    //if the num of non duplicaate mon in the evo family are greater than 6, canuse the eevee listing.
    //just a matter of actually calcing that out...

    if (base_i < iterations)
    {
        u32 personality = GetPokedexMonPersonality(currentTargetSpecies);
        LoadMonIconPalettePersonality(currentTargetSpecies, personality); //Loads pallete for current mon
            if (isEevee)
                gTasks[taskId].data[4+base_i] = CreateMonIcon(currentTargetSpecies, SpriteCB_MonIcon, 45 + 26*base_i, 31, 4, personality); //Create pokemon sprite
            else
                gTasks[taskId].data[4+base_i] = CreateMonIcon(currentTargetSpecies, SpriteCB_MonIcon, 50 + 32*base_i, 31, 4, personality); //Create pokemon sprite
...
static u8 PrintEvolutionTargetSpeciesAndMethod(u8 taskId, u16 species, u8 depth, u8 base_i_offset, u32 alreadyPrintedIcons[], u32 *base_icon_i_offSet)
{
    ...
    if (NoEvo == FALSE)
    {
        if (evolutions == NULL)
        {
            ...
            if (depth == 0) //this part works but because of recursion evolution species keeps changing, need to limit to say does NOT have pre evo
            {    
    ...
    for (i = 0; i < times; i++)
    {
        ...
        sPokedexView->sEvoScreenData.targetSpecies[base_i] = currentTargetSpecies;
        ...
        //#ifdef POKEMON_EXPANSION
            ...
            case EVO_SPECIFIC_MON_IN_PARTY:
                StringCopy(gStringVar2, gSpeciesNames[evolutions[i].param]); //mon name
