...
void HandleInputChooseMove(void)    //test new targetting setup
{
    ...
    {                                                                   //but it doesn't effect that, it just looks weird naturally with speed up on...
        ...
        if (moveInfo->moves[gMoveSelectionCursor[gActiveBattler]] == MOVE_CURSE)
        {
            ...
            && !DoesBattlerGetTypeBasedAffinity(gActiveBattler, GetBattlerAbility(gActiveBattler), gActiveBattler, GetBattlerAbility(gActiveBattler), TYPE_GHOST)) //VSONIC hopefully works need check
...
static void PlayerHandleSuccessBallThrowAnim(void)
{
    ...
    InitAndLaunchSpecialAnimation(gActiveBattler, gActiveBattler, gCatchTargetId, B_ANIM_BALL_THROW);
...
//will need to port final changes here to all controllers
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE)
    {
        ...
        {
            if (moveInfo->monType1 != TYPE_GHOST && moveInfo->monType2 != TYPE_GHOST && moveInfo->monType3 != TYPE_GHOST
            && !DoesBattlerGetTypeBasedAffinity(gActiveBattler, GetBattlerAbility(gActiveBattler), gActiveBattler, GetBattlerAbility(gActiveBattler), TYPE_GHOST))  //vsonic need test but hope works
                moveTarget = MOVE_TARGET_USER;
            else
                moveTarget = MOVE_TARGET_SELECTED;
        }
