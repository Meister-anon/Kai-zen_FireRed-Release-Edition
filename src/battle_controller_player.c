...
static void PlayerHandleBattleMoveInfo(u32 battler);
...
static void HandleInputChooseAction(u32 battler)
{
    ...
    {
        PlaySE(SE_SELECT);
        BtlController_EmitTwoReturnValues(battler, BUFFER_B, B_ACTION_MOVE_INFO, 0);
        PlayerBufferExecCompleted(battler);
    }
}

UNUSED static void UnusedEndBounceEffect(u32 battler)
{
...
//compare later
void HandleInputChooseMove(u32 battler)    //test new targetting setup
{
    u16 moveTarget;
    ...
    {                                                                   //but it doesn't effect that, it just looks weird naturally with speed up on...
        ...
        if (moveInfo->moves[gMoveSelectionCursor[battler]] == MOVE_CURSE)
        {
            ...
            && !DoesBattlerGetTypeBasedAffinity(battler, GetBattlerAbility(battler), battler, GetBattlerAbility(battler), TYPE_GHOST)) //VSONIC hopefully works need check
...
static void WaitForMonSelection(u32 battler)
{
    if (gMain.callback2 == BattleMainCB2 && !gPaletteFade.active)
    {
        if (gPartyMenuUseExitCallback == TRUE)
            ...
            BtlController_EmitChosenMonReturnValue(battler, BUFFER_B, 6, NULL);
...
static void MoveSelectionDisplayMoveNames(u32 battler)//relevant
{
    s32 i;
    struct ChooseMoveStruct *moveInfo = (struct ChooseMoveStruct *)(&gBattleResources->bufferA[battler][4]);
...
static void MoveSelectionDisplayMoveType(u32 battler)//displays type/  & move type
{
...
static void PreviewDeterminativeMoveTargets(u32 battler) //determine who targetting
//will need to port final changes here to all controllers
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE)
    {
        ...
        {
            if (moveInfo->monType1 != TYPE_GHOST && moveInfo->monType2 != TYPE_GHOST && moveInfo->monType3 != TYPE_GHOST
            && !DoesBattlerGetTypeBasedAffinity(battler, GetBattlerAbility(battler), battler, GetBattlerAbility(battler), TYPE_GHOST))  //vsonic need test but hope works
                moveTarget = MOVE_TARGET_USER;
            else
                moveTarget = MOVE_TARGET_SELECTED;
        }
