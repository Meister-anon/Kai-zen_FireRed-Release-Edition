...
void TrySetQuestLogLinkBattleEvent(void)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_LINK)
    {
        ...
        else
        {
            ...
            for (r3 = 0; r3 < 7; r3++)
            {
                r5->v1[0][r3] = gLinkPlayers[gBattleScripting.multiplayerId ^ 1].name[r3];
            }
        }
...
static void sub_812C334(s32 * a0, s32 * a1)
{
    ...
    u8 r2 = gLinkPlayers[gBattleScripting.multiplayerId].id ^ 2;
    for (r5 = 0; r5 < 4; r5++)
    {
        if (r2 == gLinkPlayers[r5].id)
            a0[0] = r5;
        else if (r5 != gBattleScripting.multiplayerId)
