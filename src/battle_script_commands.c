...
static void Cmd_setmagiccoat(void);
...
void (*const gBattleScriptingCommandsTable[])(void) =
{
    ...
    [B_SCR_OP_JUMPBASEDONABILITY]                    = Cmd_jumpbasedonability,
    ...
    [B_SCR_OP_JUMPBASEDONTYPE]                       = Cmd_typebasedjump,
    ...
    [B_SCR_OP_CHANGETYPETORESISTTARGET]     = Cmd_changetypetoresisttarget,
    ...
    [B_SCR_OP_SETMAGICCOAT]                       = Cmd_setmagiccoat,
...
{
    if (gBattleWeather == WEATHER_NONE || !HasWeatherEffect())
        return WEATHER_NONE;
    else
...
static void Cmd_adjustdamage(void)
{
    ...
    for (battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
    {
        ...
        else if (GetBattlerAbility(battlerDef) == ABILITY_STURDY && IsBattlerAtMaxHp(battlerDef))
        {
            enduredHit |= 1u << battlerDef;
...
static void Cmd_attackanimation(void)
{
    ...
    else
    {
        ...
        if (GetMoveAnimationScript(gCurrentMove) == gBattleAnimMove_ExpandingForce && moveTarget == TARGET_BOTH && CountAliveMonsInBattle(BATTLE_ALIVE_OPPOSING_SIDE, BATTLE_OPPOSITE(gBattlerAttacker) > 1))
...
static inline bool32 TrySetReflect(enum BattlerId battler)
{
    ...
    if (!(gSideStatuses[side] & SIDE_STATUS_REFLECT))
    {
        ...
        if (IsDoubleBattle() && CountAliveMonsInBattle(BATTLE_ALIVE_OPPOSING_SIDE, battler) == 2)
...
//since effect is so strong
//decide don't need extender also
//consider if properly keeps attributes
//for things like reflecting spore
//shouldn't work on grass type still
static inline bool32 TrySetMagicCoat(enum BattlerId battler)
{
    u32 side = GetBattlerSide(battler);
    if (!(gSideStatuses[side] & SIDE_STATUS_MAGIC_COAT)
    && !IsLastMonToMove(gBattlerAttacker))
    {
        gSideStatuses[side] |= SIDE_STATUS_MAGIC_COAT;

        gSideTimers[side].magicTimer = 5;

        if (IsDoubleBattle() && CountAliveMonsInBattle(BATTLE_ALIVE_OPPOSING_SIDE, battler) == 2)
            gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_SET_MAGIC_COAT_DOUBLE;
        else
            gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_SET_MAGIC_COAT_SINGLE;
...
void SetMoveEffect(enum BattlerId battlerAtk, enum BattlerId effectBattler, enum MoveEffect moveEffect, const u8 *battleScript, enum SetMoveEffectFlags effectFlags)
{
    ...
    case MOVE_EFFECT_RAGE:
        ...
        {
            if (gBattleMons[gBattlerAttacker].volatiles.rageCounter != MAX_RAGE_BOOST_COUNTER) //with curr setup would max at a move of bp 75
            {
                    gBattleMons[gBattlerAttacker].volatiles.rageCounter++; //ok would need to see if this works, since in this script setmoveeffect is before atk, this should work but idk
                //BattleScriptPush(battleScript);
    ...
    case MOVE_EFFECT_DRAGON_RAGE:
        if (gBattleMons[gBattlerAttacker].volatiles.dragonrage)
        {
        ...
        {
            gBattleMons[gBattlerAttacker].volatiles.dragonrage = TRUE; //add check for status2 rage, if set just go to next instruction
            gBattlescriptCurrInstr = battleScript;
        }
        break;
    ...
    case MOVE_EFFECT_RECOIL_HP_25: // Struggle
    {
        ...
        //not planning to use this dumb evo methods
    ...
    case MOVE_EFFECT_REMOVE_ARG_TYPE:
    {
        ...
        RemoveBattlerType(gEffectBattler, type);//vsonic look into this
        break;
    }
    ...
    case MOVE_EFFECT_ION_DELUGE:
        if (!(gFieldStatuses & STATUS_FIELD_ION_DELUGE))
        {
            ...
            gFieldTimers.IonDelugeTimer++; //check this
            BattleScriptPush(battleScript);
            gBattlescriptCurrInstr = BattleScript_MoveEffectIonDeluge;
        }//think is just plasma fist
        break;
    ...
    case MOVE_EFFECT_GRAVITY:
        ...
        break;//belive are gmax effects vsonic
    ...
    case MOVE_EFFECT_FIRE_SPIN_SIDE:
        ...
        for (enum BattlerId battler = 0; battler < MAX_BATTLERS_COUNT; ++battler)
        {
            ...
            if (!gBattleMons[battler].volatiles.wrapped)
            {
                ...
                //just comment is gmax stuff will remove anyway
    ...
    case MOVE_EFFECT_YAWN_FOE:
        if (gBattleMons[gBattlerTarget].volatiles.yawn == 0
            && CanBeSlept(gBattlerTarget, gBattlerTarget, abilities[gBattlerAttacker], abilities[gBattlerTarget], BLOCKED_BY_SLEEP_CLAUSE)
            && RandomPercentage(RNG_G_MAX_SNOOZE, 50))//ability shouldn't matter but storing ayway
        {
         ...
         && !(gBattleStruct->battlerState[battler].fainted))
        {
            if (gBattleMons[battler].volatiles.neutralizingGas)
            {
                ...
                if (!IsNeutralizingGasOnField())
                {
                    ...
                    return;
                }//add logic for immutable wind vsonic
            }
...
static void Cmd_tryselfconfusiondmgformchange(void)
{
    ...
    if (TryBattleFormChange(gBattlerAttacker, FORM_CHANGE_BATTLE_HIT_BY_CONFUSION_SELF_DMG, ability))
    {
        ...
        case ABILITY_DISGUISE:
            if (GetConfig(DISGUISE_HP_LOSS) >= GEN_8 && wasDisguised)
...
//vsonic changed to side statuss
static void Cmd_sethealblock(void)
{
    CMD_ARGS(const u8 *failInstr);
    ...
    {
        gSideStatuses[GetBattlerSide(gBattlerTarget)] |= SIDE_STATUS_HEAL_BLOCK;
        gSideTimers[GetBattlerSide(gBattlerTarget)].healBlockTimer = B_HEAL_BLOCK_TIMER;
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
}
...
//important updating battle data for switch in mon
...
static void Cmd_switchindataupdate(void)
{
    ...
    //believe checking if an item has been knocked off vsonic
    if (GetBattlerPartyState(battler)->lostItemtoKnockOff)
    {
...
static void Cmd_jumpifcantswitch(void)
{
    CMD_ARGS(enum BattlerId battler:7, u8 ignoreEscapePrevention:1, const u8 *jumpInstr);
...
static void Cmd_openpartyscreen(void)
{
    CMD_ARGS(enum BattlerId battler:7, u8 partyScreenOptional:1, const u8 *failInstr);
...
static bool32 TryCheekPouch(enum BattlerId battler, u32 itemId, const u8 *nextInstr)
{
...
static void DrawLevelUpBannerText(void)
{
    ...
    printerTemplate.color.shadow = TEXT_COLOR_DARK_GREY;
...
static void PutMonIconOnLvlUpBanner(void)
{
    ...
    iconSheet.data = GetMonIconPtr(species, personality, 1); //idk what frame is for here, do emerald icons not move? 
...
static void Cmd_tryconversiontypechange(void)
{
    ...
    //added extra protection missed before
...
//believe need update my custom stuff and for cupids arrows vsonic
static void Cmd_tryinfatuating(void)
{
    CMD_ARGS(const u8 *failInstr);
...
//vsonic need take in my custom logic
...
static void Cmd_changetypetoresisttarget(void)
{
    ...
    //would work but same thing as foresight old logic
    //only works so long as type is on specific order
...
//remember to set to side status
static void Cmd_setembargo(void)
{
    CMD_ARGS(const u8 *failInstr);

    if (gSideStatuses[GetBattlerSide(gBattlerTarget)] & SIDE_STATUS_EMBARGO)
    {
        gBattlescriptCurrInstr = cmd->failInstr;
    }
    else
    {
        gSideStatuses[GetBattlerSide(gBattlerTarget)] |= SIDE_STATUS_EMBARGO;
        gSideTimers[GetBattlerSide(gBattlerTarget)].embargoTimer = B_EMBARGO_TIMER;
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
}

//idk if will use this
//had reworked effet to multihit 
static void Cmd_presentdamagecalculation(void)
{
    CMD_ARGS();
    ...
    if (gBattleMons[gBattlerAttacker].hp != gBattleMons[gBattlerAttacker].maxHP)
    {
        ...
        else // B_TIME_OF_DAY_HEALING_MOVES == GEN_2
        {
            ...
            case EFFECT_MORNING_SUN:
                ...
                if (time == TIME_DAY)   //may make morning
                    healingModifier = 2;
                break;
...
static void Cmd_setstickyweb(void)
{
    ...
    {
        if (gSideTimers[targetSide].stickyWebAmount == 0) // Add only once to the queue
...
//vsonic update counter 2
//does it make sense to just handle this in attack canceler
//plan setup custom logic
//ok my version of effect didn't get stab
//but don't have many strong psychic moves
//so think want to do psychic damage? 
//but part of the balance was that it doesnt do super effective dmg
//or get stab
//ok will live like that for now since I'm able to toss 2
//and gain benefit of able to get around type immunity
static void Cmd_setfutureattack(void)
{
    CMD_ARGS();
    ...
    gBattleStruct->futureSight[gBattlerTarget].counter2 = 3;
...
static void Cmd_tryswapitems(void)
{
    ...
    else
    {
        ...
        if (!(gBattleTypeFlags & (BATTLE_TYPE_LINK
                             ...
                             | BATTLE_TYPE_SECRET_BASE
                             | BATTLE_TYPE_RECORDED_LINK))
            && (GetBattlerPartyState(gBattlerAttacker)->lostItemtoKnockOff || GetBattlerPartyState(gBattlerTarget)->lostItemtoKnockOff))
        {
            gBattlescriptCurrInstr = cmd->failInstr;
        }
...
{
    CMD_ARGS();
    if (!TrySetMagicCoat(gBattlerAttacker))
    {
        gBattleStruct->moveResultFlags[gBattlerTarget] |= MOVE_RESULT_MISSED;
        gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_SIDE_STATUS_FAILED;
    }
    ...
    {
        //need set elsewhere long as timer is up figure out
...
{//ok all this was almost a waste pick up doesn't work how I thought it did. -_- it doesn't have an effect on battle
    ...
    for (i = 0; i < PARTY_SIZE; ++i)
    {
        ...
        && GetPocketByItemId(heldItem) == POCKET_BERRIES
        && (Random() % 16) == 0)
        {
...
static void Cmd_givecaughtmon(void)
{
    ...
    u16 heldItem = GetMonData(caughtMon, MON_DATA_HELD_ITEM);
    u16 clearItem = ITEM_NONE;
    if (GiveMonToPlayer(caughtMon) != MON_GIVEN_TO_PARTY) //if mon going to pc, is this codeblok
    {
...
static void SetArenMonLostValues(enum BattlerId battler, u32 side)
{
    gBattleMons[battler].hp = 0;
    gHitMarker |= HITMARKER_FAINTED(battler);
...
void BS_TryPsychoShift(void)
{
    ...
    else if ((gBattleMons[gBattlerAttacker].status1 & STATUS1_SLEEP) && CanBeSlept(gBattlerAttacker, gBattlerTarget, GetBattlerAbility(gBattlerAttacker), targetAbility, BLOCKED_BY_SLEEP_CLAUSE))
    {
...
void BS_TryBestow(void)
{
    ...
    if (gBattleMons[gBattlerAttacker].item == ITEM_NONE
        ...
        || GetBattlerPartyState(gBattlerTarget)->lostItemtoKnockOff)
    {
        gBattlescriptCurrInstr = cmd->failInstr;
    }
    else
...
void BS_SetAuroraVeil(void)
{
    ...
    else
    {
        ...
        if (IsDoubleBattle() && CountAliveMonsInBattle(BATTLE_ALIVE_OPPOSING_SIDE, gBattlerAttacker) == 2)
            gBattleCommunication[MULTISTRING_CHOOSER] = 5;
        else
            gBattleCommunication[MULTISTRING_CHOOSER] = 5;
    }
...
//vsonic think make immutable wind companion function
...
static bool32 IsRototillerAffected(enum BattlerId battler, u32 move)
{
    if (IsBattlerUnaffectedByMove(battler))
        return FALSE;
    if (!IsBattlerGrounded_IgnoreException(battler, GetBattlerAbility(battler), GetBattlerHoldEffect(battler)))
...
void BS_HandleEscapePrevention(void)
{
    ...
    bool32 SelfTarget = gBattlerAttacker == battler;
...
}//can potentially wrap into abilityeffect_on_weather
//since want only happen when weather gets set
//or when switch in with weather,
//but also tail wind
...
//removed under_score in name seemed to prevent use
//since battlescript alrady sets field effect, just sets timer here
void BS_setiondeluge(void) 
{ 
    NATIVE_ARGS();
