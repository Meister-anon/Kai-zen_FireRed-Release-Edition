...
void BattleLoadOpponentMonSpriteGfx(struct Pokemon *mon, u8 battlerId)
{
    ...
    isShiny = IsShinyOtIdPersonality(otId,monsPersonality);
    if (gBattleSpritesDataPtr->battlerData[battlerId].transformSpecies == SPECIES_NONE)
...
void BattleLoadPlayerMonSpriteGfx(struct Pokemon *mon, u8 battlerId)
{
    ...
    // transform's pink color - faded color
    if (gBattleSpritesDataPtr->battlerData[battlerId].transformSpecies != SPECIES_NONE)
    {
        BlendPalette(paletteOffset, 16, 6, RGB_WHITE);
        CpuCopy32(gPlttBufferFaded + paletteOffset, gPlttBufferUnfaded + paletteOffset, 32);
...
void HandleSpeciesGfxDataChange(u8 battlerAtk, u8 battlerDef, u8 notTransform)
{
    ...
    if (notTransform == 255)
    {
        ...
        isShiny = IsShinyOtIdPersonality(otId,personalityValue);
...
void AllocateMonSpritesGfx(void) //look into if relevant for  more efficient set for cast form change
{
    u8 i = 0, j;
