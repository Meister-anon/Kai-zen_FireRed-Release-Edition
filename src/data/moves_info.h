...
const struct BattleMove gBattleMoves[MOVES_COUNT] =
{
    ...
    //ok reworked a bit hopefully still useful
    //and double check how yawn works, w my rework
    //think plan was decrement in atk cancel so its just
    //one turn one attack
    //alternately could leave target change as a cacophony boost effect
    //like speaker amp
    //stays single target but when cacophony boosted
    //it'll target opponents entire field and set max turn sleep
    //instead of yawn? or cacophony immune to ally sound moves
    //vsonic sigh idk will need test
    //ok yeah think I need to keep yawn effect to foes an ally
    //if its affecting entire enemy side would just be broken
    //without that, could just yawn shadow tag and it'd just be over
