...
SINGLE_BATTLE_TEST("Slush Rush doubles speed from snow")
{
    GIVEN {
        ...
        TURN { MOVE(opponent, MOVE_CELEBRATE); MOVE(player, MOVE_SNOW_DAY); }
        TURN { MOVE(player, MOVE_CELEBRATE); MOVE(opponent, MOVE_CELEBRATE); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_CELEBRATE, opponent);
...
{
    GIVEN {
        ASSUME(GetSpeciesPrimaryType(SPECIES_WOBBUFFET) != TYPE_ICE);
        ASSUME(GetSpeciesSecondaryType(SPECIES_WOBBUFFET) != TYPE_ICE);
