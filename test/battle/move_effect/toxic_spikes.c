...
SINGLE_BATTLE_TEST("Toxic Spikes are removed by grounded Poison-type Pokémon on switch in")
{
    ...
    GIVEN {
        ASSUME(gSpeciesInfo[SPECIES_EKANS].type1 == TYPE_POISON);
        ASSUME(gSpeciesInfo[SPECIES_ZUBAT].type1 == TYPE_POISON);
        ASSUME(gSpeciesInfo[SPECIES_ZUBAT].type2 == TYPE_FLYING);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(species) { Item(item); }
    } WHEN {
