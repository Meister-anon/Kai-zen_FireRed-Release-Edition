...
ASSUMPTIONS
{
    ...
    ASSUME(gSpeciesInfo[SPECIES_WOBBUFFET].type1 != TYPE_FLYING);
    ASSUME(gSpeciesInfo[SPECIES_WOBBUFFET].type2 != TYPE_FLYING);
...
{
    GIVEN {
        ASSUME(gSpeciesInfo[SPECIES_SKARMORY].type1 == TYPE_STEEL);
        ASSUME(gSpeciesInfo[SPECIES_SKARMORY].type2 == TYPE_FLYING);
...
SINGLE_BATTLE_TEST("Roost, if used by a Flying/Flying type, treats the user as a Normal-type (or Typeless in Gen. 4) until the end of the turn")
{
    ...
    GIVEN {
        ASSUME(gSpeciesInfo[SPECIES_TORNADUS].type1 == TYPE_FLYING);
        ASSUME(gSpeciesInfo[SPECIES_TORNADUS].type2 == TYPE_FLYING);
...
SINGLE_BATTLE_TEST("Roost, if used by a Mystery/Flying type, treats the user as a Mystery/Mystery type until the end of the turn")
{
    ...
    GIVEN {
        ASSUME(gSpeciesInfo[SPECIES_MOLTRES].type1 == TYPE_FIRE);
        ASSUME(gSpeciesInfo[SPECIES_MOLTRES].type2 == TYPE_FLYING);
...
{
    GIVEN {
        ASSUME(gSpeciesInfo[SPECIES_KECLEON].type1 != TYPE_FLYING);
        ASSUME(gSpeciesInfo[SPECIES_KECLEON].type2 != TYPE_FLYING);
...
{
    GIVEN {
        ASSUME(gSpeciesInfo[SPECIES_RAYQUAZA].type2 == TYPE_FLYING);
...
{
    GIVEN {
        ASSUME(gSpeciesInfo[SPECIES_SWELLOW].type1 == TYPE_NORMAL);
