...
SINGLE_BATTLE_TEST("Rest fails if the user is protected by Electric/Misty Terrain")
{
    ...
    GIVEN {
        ...
        ASSUME(GetSpeciesPrimaryType(SPECIES_WYNAUT) != TYPE_FLYING && GetSpeciesSecondaryType(SPECIES_WYNAUT) != TYPE_FLYING);
...
{
    GIVEN {
        ASSUME(GetSpeciesPrimaryType(SPECIES_CHIKORITA) == TYPE_GRASS || GetSpeciesSecondaryType(SPECIES_CHIKORITA) == TYPE_GRASS);
