...
#define CAN_ABILITY_ABSORB(battler) (gBattleMons[battler].status1 == 0 && !gBattleMons[battler].volatiles.rechargeTimer  && !(gBattleMons[battler].status2 & PREOCCUPIED_STATUS) && !(gStatuses3[battler] & STAUS3_VULNERABLE) && !(gBattleMons[battler].status4 & ITS_A_TRAP_STATUS4))
