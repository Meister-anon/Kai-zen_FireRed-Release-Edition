...
#define sSAVED_DMG gBattleScripting + 0x8				//s32 savedDmg; 4 byte 0x2d 0x2e 0x2f 0x30
#define sSAVED_STRINGID gBattleScripting + 0xC		    //2 bytes now /u8 twoTurnsMoveStringId; 1 byte f //becomes savedStringId in EE can be used for multiple things thinkm make 2 byte to be able store actual stringids

#define sMULTIHIT_STRING gBattleScripting + 0xE			//u8 multihitString[6]; 6 1byte  8 9 a b c d
...
#define sDMG_MULTIPLIER gBattleScripting + 0x14			//u8 dmgMultiplier; 1 byte e
#define sB_ANIM_ARG1 gBattleScripting + 0x15			//u8 animArg1; 1 byte 0x10
...
#define sSAVED_MOVE_EFFECT gBattleScripting + 0x1E		//u16 savedMoveEffect; 2 byte 0x18 0x19         // savedMoveEffect
#define sMOVE_EFFECT gBattleScripting + 0x20			//u16 moveEffect; 2 byte 0x1a 0x1b          // moveEffect   //these really are move effect
#define sB_ANIM_TURN gBattleScripting + 0x22			//u8 animTurn; 1 byte 0x1d
#define sB_ANIM_TARGETS_HIT gBattleScripting + 0x23		//u8 animTargetsHit; 1 byte 0x1e
#define sSTATCHANGEID gBattleScripting + 0x24           //statChangeId new value for dynamic stat set, this stat id will be passed to statchanger stat argument
#define sSTATCHANGER gBattleScripting + 0x25			//u8 statChanger; 1 byte 0x1f
#define sSTAT_ANIM_PLAYED gBattleScripting + 0x26		//bool8 statAnimPlayed; 1 byte 0x20
#define sGIVEEXP_STATE gBattleScripting + 0x27			//u8 atk23_getexpState; 1 byte 0x21
#define sBATTLE_STYLE gBattleScripting + 0x28			//u8 battleStyle; 1 byte 0x22   //switch or set
#define sLVLBOX_STATE gBattleScripting + 0x29			//u8 atk6C_drawlvlupboxState; 1 byte 0x23
#define sLEARNMOVE_STATE gBattleScripting + 0x2A		//u8 learnMoveState; 1 byte 0x24
#define sPURSUIT_DOUBLE_DAMAGE gBattleScripting + 0x2B	//u8 field_25_pursuitDoublesAttacker; 1 byte 0x25    //unsure of use  maybe i changed but does'nt seem used
#define sRESHOW_MAIN_STATE gBattleScripting + 0x2C		//u8 reshowMainState; 1 byte 0x26
#define sRESHOW_HELPER_STATE gBattleScripting + 0x2D	//u8 reshowHelperState; 1 byte 0x27
#define sSAVED_STAT_CHANGER gBattleScripting + 0x2E		//u8 savedStatChanger; 1 byte 0x28	for moody

#define sSAVED_BATTLER (gBattleScripting + 0x2F)		//u8 savedBattler; 1 byte 0x29
...
#define sSWITCH_CASE (gBattleScripting + 0x30)			//u8 switchCase; 1 byte 0x2a    // switchCase  used for red card special swith condition	//FORCED SWITCH?
#define sBERRY_OVERRIDE	(gBattleScripting + 0x31)		//u8 overrideBerryRequirements; 1 byte 0x2b  // overrideBerryRequirements  //ok this seems to just mean don't consume berry???  
#define sSTICKY_WEB_STAT_DROP (gBattleScripting + 0x32) //u8 stickyWebStatDrop; 1 byte 0x2c  // overrideBerryRequirements
...
#define sLVLUP_HP gBattleScripting + 0x33				//u8 field_23;	1 byte 0x31		//nothing in struct below this think all used values must go above here
#define sWINDOWS_TYPE gBattleScripting + 0x34
#define sMULTIPLAYER_ID gBattleScripting + 0x35
#define sSPECIAL_TRAINER_BATTLE_TYPE gBattleScripting + 0x36    //wait aren't these limited to 32?

//after moving things to optimize placement of even bits
//somehow I have more space? I must have counted wrong initially
//maybe thats why stats broke not bad code but mis alignment
